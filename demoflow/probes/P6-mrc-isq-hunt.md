...
- Geography column located by a header cell BEGINNING `MRC` at row 2, column 1 (0-indexed); the cell reads `MRC`. Prefix, not equality and not substring, and the reason is visible in this run's own evidence: the §3b table lists the geography header EVERY opened candidate publishes, and the editions heading it with an RA qualifier would be missed by an equality test, while a substring test locks onto the caption row (cell A1 above, which also contains "mrc") and counts zero labels below it. No edition or year is named here — the table is what states which files have which header.
