...
scaling of ρ cancels exactly in ED. This run measured ρ's contamination in the model's own
lattice AS OF 2026-08-15 — 98-10-0231-01 (the cube `loaders/census.py` reads) and its CSD
sibling 98-10-0232-01, on the FOUR bands `census._AGE_BAND_SPEC` then carried
(25-54/55-64/65-74/75+) — rather than on a proxy. The band-uniform premise is measured FALSE in
its strict form and the residual it leaves is measured second-order; both halves are published,
because the premise is what the ruling rests on and the size is what makes the ruling survive.

THAT LATTICE IS NOW A DATED RECORD, NOT THE LIVE ONE. Operator ruling W (2026-08-20) refined the
model's ownership lattice to seven bands; this file is deliberately NOT re-run for it, and
`RHO_BANDS_CMA` below carries the full argument plus the test that holds the two lattices in an
exact-coarsening relation. Every sentence and every emitted DECISION token in this file describes
the 2026-08-15 measurement, so none of them is refreshed in place — a re-measurement is a new
run with its own vintage pin.
...
launder itself into `pytest.skip`):
  ...
  * `_guard_citation`        — every figure this run REPRODUCES from §6 and from the public
                               Task-26 bracket record matches those documents' own digits.
...
BRACKET_RECORD = (_REPO_ROOT / "docs" / "research"
                  / "2026-08-15-task26-hors-residual-bracket-record.md")
...
# The model's own bands AS OF THIS RUN (2026-08-15), expressed in each cube's published age
# members. The band EDGES were `census._AGE_BAND_SPEC`'s; the constituents are whatever each cube
# publishes, and the two cubes publish different granularities of the same partition.
#
# FROZEN, NOT STALE — DO NOT "REFRESH" THESE TO THE LIVE SPEC. Operator ruling W (2026-08-20;
# spec §8 amendment #18) refined the model's ownership lattice to SEVEN bands
# (25-34/35-44/45-54/55-64/65-74/75-84/85+), so these four no longer equal it. They are kept
# because the ρ table they produce is the WARRANT for reversing spec §6 amendment #12(B):
# `loaders/hors_aligned.py` records the 1.202 pp four-band spread as the reversal's history, and
# `tests/test_hors_aligned_ownership.py` pins it as `_P10_FOUR_BAND_SPREAD_PP` and asserts the
# refined lattice's WORST feasible spread EXCEEDS it — the check that makes "the refinement
# strengthened the reversal" measured rather than narrated. Re-running this probe at seven bands
# would overwrite the very number that comparison is against, turning it into the spread against
# itself and leaving the reversal's warrant unauditable. (The refutation itself got STRONGER on
# the refined lattice — 3.318 pp against this run's 1.202 — which is measured in
# `loaders/hors_aligned.py`, not here.)
#
# WHAT HOLDS THE TWO LATTICES TOGETHER instead of equality: `tests/test_probe_p10.py` asserts
# these four bands are an EXACT COARSENING of the live seven — same outer edges, and each band's
# cube members exactly the live members tiled across its span, checked on BOTH cubes
# (`census._AGE_BAND_SPEC` for the CMA side, `hors_aligned.CD_BAND_SPEC` for the CD side). That
# reds if the live lattice moves to something this run's figures cannot be compared against,
# while staying green under a pure refinement.
RHO_BANDS_CMA = {
...
def _bracket_record() -> str:
    """The public Task-26 bracket record (spec amendment #29) — where the whole-CD

    bracket's other variants and the shipped settled-persons figure are stated. It lives in
    docs/research/ so the shipped test suite reads nothing outside what ships; it replaced
    the internal dispatch record this read previously resolved to.
    """
    if not BRACKET_RECORD.exists():
        raise ProbeRefusal("bracket-record",
                           f"{BRACKET_RECORD.name} is absent; this note may not reproduce bracket "
                           "figures it cannot read at their source")
    return BRACKET_RECORD.read_text(encoding="utf-8")
...
def _sections(where: list) -> list:            # noqa: C901 — one derivation, read top to bottom
    ...
    _guard_citation(f"the public Task-26 bracket record ({BRACKET_RECORD.name})", qfe,
                    qfe_coupled)
    ...
    lines += [
        ...
        # FROZEN PROSE, like every DECISION token below: it describes the lattice THIS run
        # measured on (see `RHO_BANDS_CMA`). Ruling W's seven-band refinement does NOT edit it —
        # only a re-run at a new vintage pin does.
