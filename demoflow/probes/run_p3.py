...
def _catalogue() -> list:
    """GET `getAllCubesListLite`. Boundary `wds-list` (www150).

    A named seam for the one boundary this probe does not reach through `_post`, so the
    offline regen gate can drive the sweep's title predicate over a recorded catalogue
    instead of skipping `_sweep` wholesale. No try/except and no sentinel: `_sweep`'s
    caller distinguishes "the catalogue was searched" from "the sweep failed", and a
    swallowed error here would publish a NOT-FOUND-shaped record of a search that never ran.
    """
    return json.loads(urllib.request.urlopen(WDS_LIST, timeout=WDS_TIMEOUT).read())
