...
T13b (2026-08-08, the DIV re-triage discharge record)
adds three blocks:
...
def test_mtl_rmr_75plus_derivation_matches_spec_oracle():
    """The spec's own provenance figure, still anchoring the producer AFTER the lattice split.

    THE ANCHOR WAS THE POINT, NOT THE BAND SHAPE (operator ruling W, 2026-08-20). The spec
    quotes 56.2% = 113,730 / 202,535 = 0.56153... for MTL at 75+, and until this change the
    producer served exactly one rate over that age range, so `age=78` read it directly. The
    lattice now SPLITS 75+ into `75-84` and `85+`, so no single served rate is that figure any
    more — and deleting the gate would drop the spec's only ownership anchor at the tail.

    It is re-anchored on the PARTITION IDENTITY instead, which is a stronger claim than the
    single lookup was: the two new rates, weighted by the test-owned household counts, must
    re-aggregate to the published union. Both directions are asserted — the union reproduces
    the spec's 3-sig-fig figure, and the split is NOT vacuous (the two bands are materially
    apart, which is the gradient the flat 75+ rate hid and the reason ruling W split it).
    """
    counts = _independent_band_counts()[_GEO_SOURCE[Geography.MTL_RMR]]
    union = _independent_band_counts({"75+": _RETIRED_75PLUS_MEMBERS})[
        _GEO_SOURCE[Geography.MTL_RMR]]["75+"]
    # The spec's own two figures, transcribed: the union is the cell the spec quotes.
    assert union == (113_730, 202_535), (
        f"the extract's MTL 75+ union is {union}, not the spec's quoted 113,730 / 202,535")
    # THE TWO ASSOCIATIVITY ASSERTS THAT STOOD HERE WERE DELETED (2026-08-21 audit):
    #   assert sum(counts[b][0] for b in _RETIRED_75PLUS_BANDS) == union[0]   # and [1]
    # Both sides were integer sums of the SAME member cells read by the SAME reader
    # (`_independent_band_counts`), differing only in how the bands were GROUPED — and the two
    # groupings come from test-owned literals in this file that name the same two members
    # (`_BAND_MEMBERS["75-84"]`/`["85+"]` against `_RETIRED_75PLUS_MEMBERS`). So the equality is
    # associativity of integer addition, not a tiling check: measured across all SEVEN GEO rows
    # of the committed extract, 14 of 14 cells identical, and no extract could make them differ.
    # THE ENTAILMENT WORTH RECORDING — what actually carries the tiling claim, all three of them
    # comparing against something OUTSIDE this file: `union == (113_730, 202_535)` (the spec's
    # transcribed cell), `round(union[0] / union[1], 3) == 0.562`, and the load-bearing one,
    # `round(_band_weighted(served, counts, _RETIRED_75PLUS_BANDS), 3) == 0.562`, which weights
    # the PRODUCER's served rates by these counts and lands on StatCan's published figure. An
    # inexact split shows up there as a moved third decimal, which is the check the comment above
    # promises and the deleted pair only appeared to make.

    # THE ANCHOR, carried to the producer through the identity. `round(·, 3)` because the spec
    # figure is 3 sig figs — an abs=1e-6 comparison against a TYPED 0.562 would fail the true
    # derivation, and hand-tuning the CSV to hit an over-precise literal is the inverse error.
    rates = load_ownership_rates()
    served = {b: ownership_rate(rates, Geography.MTL_RMR, age=a)
              for b, a in (("75-84", 78), ("85+", 90))}
    assert round(_band_weighted(served, counts, _RETIRED_75PLUS_BANDS), 3) == 0.562
    assert round(union[0] / union[1], 3) == 0.562

    # NON-VACUITY: if the split were degenerate the identity above would hold for a producer
    # that simply served the old flat rate at both bands. It does not — 75-84 sits ABOVE the
    # union and 85+ BELOW it, and they are more than a rounding step apart. That gradient is
    # what ruling W split this band to expose.
    assert served["75-84"] > union[0] / union[1] > served["85+"], (
        f"the two tail bands no longer bracket the published union: {served}")
    assert round(served["75-84"], 3) != round(served["85+"], 3), (
        "the tail split is degenerate at 3 decimals, so the identity above would be satisfied "
        "by a producer still serving one flat 75+ rate")
...
def test_out_of_unit_ownership_rate_raises():
    ...
    """
    # The band key is `75-84` since ruling W split the tail (2026-08-20): age 80 resolves
    # there, and a table keyed on the retired `75+` would refuse for the WRONG reason ("no
    # ownership rate for band 75-84") and never reach the fraction guard this gate is about.
    with pytest.raises(LoaderError, match=r"\[0, ?1\]|fraction"):
        ownership_rate({g.value: {"75-84": 1.5} for g in Geography}, Geography.MTL_RMR, age=80)


def test_headship_curve_covers_all_ages_and_is_fraction():
    hs = load_headship_rates(shape=census.HEADSHIP_CENTRAL_SHAPE)
...
#   - _SPEC_AGE_BANDS      spec §8 model bands AS AMENDED — SEVEN of them (operator ruling W,
#                          2026-08-20; amendment #18). Until that ruling this tuple read
#                          `25-54 / 55-64 / 65-74 / 75+`, and its 25-54 band was thirty years
#                          wide: the producer served MTL 0.5120 at every age 25..54 while the
#                          extract's own `25 to 29 years` cells publish 0.2596. These literals
#                          therefore LEAD the spec body until amendment #18 lands beside them in
#                          the same commit (the standing #14 rule); they are transcribed from
#                          the RULING, which is the authority the amendment records.
#   - _BAND_MEMBERS        probe P2 §5's 15-member age list, allocated to those bands. Every
#                          band is an EXACT union of published members, and the retired 75+
#                          band's two constituents are now one band each.
#   - _RETIRED_75PLUS_MEMBERS  the members the retired `75+` band unioned, kept as an ORACLE
#                          UNIT rather than as a lattice entry: the external anchors below were
#                          published against that union, and re-aggregating the two new bands
#                          to it is what proves the split is EXACT rather than a remodel.
...
_SPEC_AGE_BANDS = ("25-34", "35-44", "45-54", "55-64", "65-74", "75-84", "85+")
...
# The retired `75+` band, as an ORACLE UNIT and never a lattice entry. The external published
# anchors in this file were retrieved against this union, so the union is how they still reach
# the producer: `_band_weighted` re-aggregates the two new bands to it, and an inexact split
# would show up there as a moved third decimal on a figure StatCan published.
_RETIRED_75PLUS_MEMBERS = ("75 to 84 years", "85 years and over")
_RETIRED_75PLUS_BANDS = ("75-84", "85+")
_GEO_SOURCE = {
...
def _band_weighted(rates: dict, counts: dict, bands) -> float:
    """The household-weighted mean of `bands`' rates — i.e. the rate of their UNION.

    THE PARTITION-INVARIANCE LEG, and it entails the PRODUCER rather than restating it: the
    weights are the test-owned counts, the rates are whatever the producer served, and the
    result is compared against a figure published OUTSIDE this repo. It is exact rather than
    approximate because the bands are exact unions of published members — a producer whose
    split of the retired 75+ band leaked or double-counted a member reds here even though each
    of its two rates is still a plausible fraction.
    """
    num = sum(rates[b] * counts[b][1] for b in bands)
    den = sum(counts[b][1] for b in bands)
    return num / den


def _expected_rate_table() -> dict[str, dict[str, float]]:
    """The FULL 8-geography x 7-band table, built only from the test-owned literals above."""
...
#                                  (DIV discharge record, lines 22-25),
...
def test_headship_curve_matches_a_full_band_independent_recompute():
    ...
    for band, reference in _DIV_REFERENCE_HEADSHIP.items():
        assert round(expected[band], _DIV_REFERENCE_DP) == reference, (
            ...
            f"{reference} (DIV discharge record, lines 22-25)")
...
def test_headship_zero_support_band_records_why_it_is_kept():
    ...
    # BOUND TO THE MEMBER, not present anywhere. `f"{maintainers['0-19']:,} households" in note`
    # stood here and the note states that count TWICE — once as the youngest published member's
    # size and once inside the 15-19 owner/total pair — so dropping the occurrence that names the
    # member left the other and shipped green. That is the occurrence-selective evasion: the
    # digits survive while the clause that gives them their role does not.
    assert (f"the youngest of which is '15 to 19 years' at {maintainers['0-19']:,} households"
            in " ".join(note.split())), (
        f"zero_support_note no longer names the youngest published member BESIDE its count "
        f"({maintainers['0-19']:,} households) — the same digits appear again inside the 15-19 "
        f"owner/total pair, so their presence is not this clause: {note!r}")
    assert f"{under_15:,.0f} persons aged 0-14" in note, (
        f"zero_support_note lost its role-bound denominator: {note!r}")
    # ONE denominator for that role: the contiguous clause above closes a swap and a drop, never
    # an ADDITION, so a second `N persons aged 0-14` beside the true one would satisfy it.
    # IGNORECASE (run 48): the ADDITION this leg exists to catch is invisible to a
    # case-sensitive pattern, so ", or 9,999,999 PERSONS AGED 0-14" left the set a singleton.
    denominators = set(re.findall(r"([\d,]+) persons aged 0-14", " ".join(note.split()),
                                 re.IGNORECASE))
    assert denominators == {f"{under_15:,.0f}"}, (
        f"zero_support_note attaches {sorted(denominators)} to the under-15 person count; this "
        f"extract publishes {under_15:,.0f} — the closure bound is stated against ONE "
        "denominator or it bounds nothing")
    ...
    assert not says(note, "no published maintainer member under 15"), (
...
# TEST-OWNED literals, transcribed from the QFE record (SUBJECT 2(a) of the QFE
# Task-26 re-triage record) and never read from it at runtime —
    ...
    for geo, cells in _QFE_SUB_FLOOR_CELLS.items():
        for member, expected in cells.items():
            assert counts[geo][member] == expected, (
                ...
                f"{expected} (QFE Task-26 re-triage record)")
...
def test_zero_support_note_calls_the_sub_25_zero_a_CHOICE_and_cites_what_is_published():
    ...
    """
    cells = _independent_band_counts({m: (m,) for m in _SUB_FLOOR_MEMBERS})[_PROVINCE]
    # EVERY sub-floor cell the note quotes, bound to the MEMBER it belongs to and in the order
    # the note states them. `f"{owner:,} owners of {total:,} households" in note` stood here and
    # asked about the 20-24 cell alone: the note quotes TWO members, so the 15-19 pair was
    # ungated (a wrong cell there shipped green) and a THIRD, false pair added beside them
    # satisfied the same assert. One list equality covers all three evasions — a swap between
    # the two members, a dropped member, and an added one.
    want_cells = [(m, f"{cells[m][0]:,}", f"{cells[m][1]:,}") for m in _SUB_FLOOR_MEMBERS]
    fresh = derive_headship_from_sources(DATA_DIR / CENSUS_EXTRACT, DATA_DIR / POP_QC_WORKBOOK)
    ...
    for surface, provenance in (("committed", _committed_headship()["_provenance"]),
        ...
        quoted = re.findall(r"'(\d+ to \d+ years)' ([\d,]+) owners of ([\d,]+) households",
                            " ".join(note.split()))
        assert quoted == want_cells, (
            f"{surface}: the note quotes the published sub-25 cells as {quoted}; the extract "
            f"publishes {want_cells}. Each cell must be cited in role-bound form BESIDE THE "
            f"MEMBER IT IS FROM — a pair present but attached to the other member, or a third "
            f"pair added beside the two, is what a presence check passes: {note!r}")
        # AND THE CELL-LABEL SHAPE, COUNTED IN THE SURFACE, because the list equality above is
        # PHRASE-EXACT. An added cell written one word differently — "'25 to 29 years' has
        # 40,000 owners among 150,000 households" rather than "owners of" — matches no group of
        # that regex at all, so the equality holds and the false citation ships. Measured GREEN
        # 2026-08-21 through `gen_headship.py`, so the regen-equality gate was satisfied too.
        # This note names a member-age band in exactly five places: the cells cited above, the
        # youngest-published-member clause, and the two `_HEADSHIP_MEMBER_SPEC` mentions.
        labelled = re.findall(r"'\d+ to \d+ years'", " ".join(note.split()))
        assert len(labelled) == len(want_cells) + 3, (
            f"{surface}: zero_support_note names {len(labelled)} member-age bands {labelled}; "
            f"this note has {len(want_cells) + 3} roles for one — {len(want_cells)} cited "
            "cells, the youngest published member, and the two `_HEADSHIP_MEMBER_SPEC` "
            "mentions. A band label with no role is a cell citation the equality above cannot "
            f"see: {note!r}")
...
def test_stale_headship_artifact_is_refused_at_load(tmp_path):
    ...
    for field, mutate in (
        ...
        with pytest.raises(LoaderError, match="sha256") as exc:
            load_headship_rates(data_dir=tmp_path, shape=census.HEADSHIP_CENTRAL_SHAPE)
...
def test_hors_rmr_is_province_net_of_all_six_cmas():
    ...
    RUN OVER EVERY BAND since ruling W (2026-08-20), where it used to read the retired `75+`
    one alone. The netting property is band-INDEPENDENT, so the single-band form was a
    narrowing with no reason behind it — and the finer lattice makes the discriminator sharper
    rather than weaker: the wrong netting must be materially wrong at all SEVEN bands, not at
    one.
    """
    table = _independent_band_counts()
    assert set(table) == {_PROVINCE, *_ALL_QC_CMAS}, (
        f"extract GEO set drifted: {sorted(table)}")

    rates = load_ownership_rates()
    fresh = derive_ownership_from_csv(DATA_DIR / CENSUS_EXTRACT)["rates"]
    for band in _SPEC_AGE_BANDS:
        counts = {geo: bands[band] for geo, bands in table.items()}
        ...
        assert all_t > 0 and 0 <= all_o <= all_t, f"{band}: residual left the feasible region"
        ...
        # moves the artifact with it, so the no-drift gate goes green and cannot be the
        # backstop. Measured 2026-08-08 (regenerate-through battery): the `fresh` leg below and
        ...
        lo, hi = next((lo, hi) for lbl, lo, hi in census._AGE_BANDS if lbl == band)
        assert ownership_rate(rates, Geography.HORS_RMR, age=lo) == pytest.approx(
            expected, rel=1e-12), band
        assert fresh[Geography.HORS_RMR.value][band] == pytest.approx(expected, rel=1e-12), band

        two = ("Montréal (CMA), Que.", "Québec (CMA), Que.")
        two_cma_o = prov_o - sum(counts[c][0] for c in two)
        two_cma_t = prov_t - sum(counts[c][1] for c in two)
        assert abs(two_cma_o / two_cma_t - expected) > 1e-3, (
            f"{band}: netting only MTL+QC must give a MATERIALLY different rate, else this "
            "gate is vacuous")
...
def test_isq_territory_note_binds_each_figure_to_its_role():
    ...
    # AND EXCLUSIVITY, the leg a role clause alone does not carry (2026-08-21 class sweep). A
    # contiguous clause makes a SWAP red and a DROP red; it does not make an ADDITION red, so
    # the note could carry every correct clause and a second, false one of the same shape
    # beside it. Each role phrase may name exactly ONE figure.
    # The patterns are anchored on the ROLE PHRASE ALONE, never on the arithmetic that precedes
    # it: measured 2026-08-21, a leading `= ` / `row ` anchor let a second clause written without
    # that prefix — ", or 2,384,575 persons (this rate's territory)" — escape the count.
    for role, pattern in (("this rate's territory",
                           r"([\d,]+) persons \(this rate's territory\)"),
                          ("the population the rate multiplies",
                           r"([\d,]+) persons \(the population the rate multiplies\)"),
                          ("the Ottawa-Gatineau Québec-part gap",
                           r"([\d,]+)-person gap is exactly the ISQ Ottawa-Gatineau")):
        # IGNORECASE for the same reason the count below is: an addition written in the
        # emphasis capitals this corpus uses — ", or 9,999,999 PERSONS (THIS RATE'S TERRITORY)"
        # — is invisible to a case-sensitive pattern, so BOTH this leg and the shape count
        # below stayed at their baseline and the second figure shipped (run 48).
        found = set(re.findall(pattern, " ".join(note.split()), re.IGNORECASE))
        assert len(found) == 1, (
            f"isq_territory_note attaches {sorted(found)} to {role!r} — exactly one figure may "
            "carry that role, or a reader cannot tell which of two the join should use")
    # AND THE PERSON-FIGURE SHAPE, COUNTED IN THE SURFACE. The three role patterns above are
    # PHRASE-EXACT, so the same false claim written WITHOUT the role's parenthetical — ", or
    # 9,999,999 persons for this rate's territory" — is invisible to every one of them and to
    # the clause legs as well. Measured GREEN 2026-08-21 through `gen_ownership.py`. The count
    # is exact rather than approximate here: this note states `N persons` in exactly the two
    # places above, and its third figure is written as a `-person gap`.
    stated = re.findall(r"[\d,]+ persons", " ".join(note.split()), re.IGNORECASE)
    assert len(stated) == 2, (
        f"isq_territory_note states {len(stated)} person counts {stated}; it has two roles for "
        "one — this rate's territory, and the population the rate multiplies. A third is a "
        f"figure with no role, which the phrase-exact patterns above cannot reach: {note!r}")
...
def test_a_half_registered_raw_anchor_refuses_instead_of_crashing(tmp_path, monkeypatch):
    ...
    for name, payload, load in (
            ...
            (HEADSHIP_ARTIFACT, _committed_headship(),
             lambda data_dir: load_headship_rates(
                 data_dir=data_dir, shape=census.HEADSHIP_CENTRAL_SHAPE))):
        payload["_provenance"]["raw_source_sha256"] = "0" * 64
...
def test_absent_statistic_or_total_member_names_the_member_and_not_the_geographies(
    ...
    for original, renamed in mutants:
        ...
        assert not says(message, "GEO set") and _GEO_SOURCE[Geography.MTL_RMR] not in message, (
...
def _published_cell(geo: str, member: str) -> tuple[int, int]:
    """(owner, total) for ONE published age member — a MODEL BAND since ruling W.

    Both tail members are now bands of their own (`75-84`, `85+`), so each external cell
    anchors a served rate DIRECTLY instead of only through a union. That is a strengthening the
    lattice refinement handed us: two anchored cells where there was one anchored union.
    """
    return (_PUBLISHED_CELLS[(geo, member, "Owner")],
            _PUBLISHED_CELLS[(geo, member, "Total - Tenure")])


def _published_75plus(geo: str) -> tuple[int, int]:
    """(owner, total) for the RETIRED 75+ union, summed from its two PUBLISHED constituents.

    Kept because the external anchors were retrieved against this union and the union is what
    the partition-invariance legs re-aggregate to; it is NOT a lattice entry (see
    `_RETIRED_75PLUS_MEMBERS`).
    """
    return tuple(sum(v) for v in zip(*(_published_cell(geo, m)
                                       for m in _RETIRED_75PLUS_MEMBERS)))
...
def test_qc_cma_75plus_rate_matches_the_externally_published_statcan_cells():
    ...
    4,405 / total 10,000, so the retired 75+ union is 24,560 / 46,005 = 0.5338550... -> 0.534.

    SINCE RULING W (2026-08-20) BOTH OF THOSE CELLS ARE MODEL BANDS. The anchor therefore
    reaches the producer TWICE directly — 0.5598 at 75-84 and 0.4405 at 85+ — and the union
    stays as the partition-invariance leg. That 11.9pp gap between two adjacent published cells
    is what the retired flat 0.5339 hid, in the model's most aging-sensitive region, and it is
    now asserted rather than averaged away.
    ...
    per_band = {band: _published_cell(_QC_CMA_LABEL, member)
                for band, member in zip(_RETIRED_75PLUS_BANDS, _RETIRED_75PLUS_MEMBERS)}

    # Leg 1 — exact counts, PER BAND. Immune to any rounding coincidence: the committed
    # extract's own QC CMA tail cells must BE the published pairs, digit for digit.
    counts = _independent_band_counts()
    for band, published in per_band.items():
        assert counts[_QC_CMA_LABEL][band] == published, (
            f"the committed extract's Québec CMA {band} cell {counts[_QC_CMA_LABEL][band]} is "
            f"not StatCan's published {published} — the extract was cut from a different "
            f"vintage or a different address ({_WDS_CITATION})")
    # THE UNION PAIR, against a THIRD test-owned literal rather than against itself. What stood
    # here was `tuple(sum(v) for v in zip(*per_band.values())) == (owner, total)`, which cannot
    # fail: `owner, total` come from `_published_75plus`, which is that same elementwise sum over
    # those same two `_published_cell` tuples in that same order — f(x) == f(x), proven green
    # under two separate cell corruptions (2026-08-21 audit). ENTAILMENT OF WHAT IT MEANT TO SAY:
    # leg 1 above already pins each band's committed counts against its published literal
    # digit-for-digit, and the sum of two exact equalities is exact, so the union equality had
    # nothing left to add. What is NOT entailed — and is what this replaces it with — is that
    # `_published_75plus`'s own aggregation is right and that the figure this docstring quotes
    # (24,560 / 46,005) is the sum: a `zip` over the wrong axis, or a drifted docstring, reds
    # here and reds nowhere else.
    assert (owner, total) == (24_560, 46_005), (
        f"the retired 75+ union sums to {(owner, total)}, not the 24,560 / 46,005 this test's "
        f"docstring quotes off {_WDS_CITATION}")

    # Leg 2 — the mandated round-3 rate form, at both consumer surfaces (loader + producer),
    # once per anchored band, PLUS the union the spec-era anchor was published against.
    rates = load_ownership_rates()
    fresh = derive_ownership_from_csv(DATA_DIR / CENSUS_EXTRACT)["rates"]
    for band, (o, t), age in ((b, per_band[b], a)
                              for b, a in zip(_RETIRED_75PLUS_BANDS, (78, 90))):
        assert round(ownership_rate(rates, Geography.QC_RMR, age=age), 3) == round(o / t, 3), band
        assert fresh[Geography.QC_RMR.value][band] == pytest.approx(o / t, rel=1e-12), band
    # THE PARTITION-INVARIANCE LEG IS DELETED HERE, and the entailment is recorded because its
    # MESSAGE claimed something false (2026-08-21 audit). It asserted
    #   round(_band_weighted(fresh[QC], counts[QC], _RETIRED_75PLUS_BANDS), 3)
    #       == round(owner / total, 3)
    # saying it catches a split that "leaked or double-counted a member while both rates stayed
    # plausible fractions". It cannot: leg 1 pins `counts[QC][band]` to the published integers
    # EXACTLY, and the loop above pins `fresh[QC][band]` to `o/t` at rel=1e-12, and
    # `_band_weighted` is sum(rate_b * total_b) / sum(total_b) — so it reduces to
    # sum(o_b)/sum(t_b) = owner/total identically, to 1e-12, before any rounding. A leaking
    # producer reds on the per-band legs first and strictly harder. The invariance property
    # itself is NOT abandoned: `test_mtl_rmr_75plus_derivation_matches_spec_oracle`
    # carries it at MTL against the EXTERNAL 3-sig-fig 0.562, where the per-band rates are read
    # off the loader and are not pinned to the union's own constituents, so there the identity
    # does discriminate.

    # Leg 3 — the round-3 tolerance actually DISCRIMINATES the wrong-CMA mutation, at BOTH
    # anchored bands rather than at one union.
    for band, (o, t) in per_band.items():
        mtl_owner, mtl_total = counts[_GEO_SOURCE[Geography.MTL_RMR]][band]
        assert round(mtl_owner / mtl_total, 3) != round(o / t, 3), (
            f"MTL and QC {band} round to the same three decimals, so the round-3 anchor above "
            "cannot discriminate the wrong-CMA mutation — this gate needs a tighter tolerance")
...
def test_hors_rmr_netting_identity_is_anchored_by_the_published_province_cell():
    ...
    and 85+ owner 55,745 / total 103,640, so the retired 75+ union is 258,575 / 439,810. Since
    ...
    # PER BAND, then at the union: `published` is EXTERNAL, the six CMA terms are committed.
    residual = {}
    for band, member, age in zip(_RETIRED_75PLUS_BANDS, _RETIRED_75PLUS_MEMBERS, (80, 90)):
        prov_owner, prov_total = _published_cell(_PROVINCE, member)
        cma_owner = sum(counts[c][band][0] for c in _ALL_QC_CMAS)
        cma_total = sum(counts[c][band][1] for c in _ALL_QC_CMAS)
        hors_owner, hors_total = prov_owner - cma_owner, prov_total - cma_total
        assert hors_total > 0 and 0 <= hors_owner <= hors_total, (
            f"{band}: residual left the feasible region")
        residual[band] = (hors_owner, hors_total)

        # Leg 1 — it must reproduce the residual the derivation actually forms, count for count.
        committed_prov_owner, committed_prov_total = counts[_PROVINCE][band]
        assert (hors_owner, hors_total) == (committed_prov_owner - cma_owner,
                                           committed_prov_total - cma_total), (
            f"the published province {band} cell {(prov_owner, prov_total)} does not reproduce "
            f"the derivation's HORS_RMR residual: the committed extract's province row is "
        ...
        assert ownership_rate(rates, Geography.HORS_RMR, age=age) == pytest.approx(
            hors_owner / hors_total, rel=1e-12), band
        assert fresh[Geography.HORS_RMR.value][band] == pytest.approx(
            hors_owner / hors_total, rel=1e-12), band

    # LEG 2b IS DELETED, and the deletion is the finding (audit, 2026-08-21). It asserted the
    # retired union's netting identity — sum of the two per-band residuals against the union
    # residual — and claimed to prove "no member leaked between the two bands". It could not:
    # `_published_75plus` is itself the SUM of the same two published member cells this loop
    # already read, and `counts` is one dict, so both sides reduced to the same integers added
    # in a different order. That equality is associativity — exact for every possible input,
    # including an input where a member HAD leaked. The claim it was reaching for is a MEMBER
    # PARTITION claim, and that one can fail and is already checked, on the test-owned oracle:
    # `test_model_band_lattices_are_spec_labelled_and_partition_their_domain` holds
    # `{label: members for ... in census._AGE_BAND_SPEC} == _BAND_MEMBERS`, so a member moved
    # between `75-84` and `85+`, dropped, or duplicated REDS there. Nothing is left uncovered by
    # the removal, and a check that cannot fail was hiding that fact.
...
def test_the_zero_support_note_keeps_THREE_claims_distinct():
    ...
    assert not says(note, "the table is silent") and not says(note, "proved exactly zero")
...
def test_the_DISCHARGED_clause_names_the_retired_SENTENCE_and_the_COMMIT_that_retired_it():
    ...
    for surface, provenance in (("committed", _committed_headship()["_provenance"]),
        ...
        assert "ruling-V commit" in note, (
...
def _assert_lattice_floor_unmoved(spec_floor: int, mirrored_floor: int) -> None:
    ...
    assert spec_floor == mirrored_floor == 25, (
        ...
        "first half (the age-resolved headship curve) landed at the ruling-V curve commit. Before the floor moves, "
...
def test_the_ownership_LATTICE_FLOOR_IS_A_TRIPWIRE_that_reds_when_the_floor_moves():
    ...
    the ruling-V curve commit, and no runtime check can express "was this measured".
...
def test_the_SOLE_TRIGGER_claim_is_SCOPED_on_every_surface_that_makes_it():
    ...
    for surface, text in (("the tripwire's failure message", _tripwire_message()),
        ...
        # CASE-FOLDED, and that is the whole leg (run 48). Written as a bare `not in`, this
        # forbid missed the casing its own FAILURE MESSAGE renders the claim in: "that tripwire
        # is the ONLY MECHANICAL TRIGGER and the only guard that reds on ANY move" satisfies
        # this leg AND the scope leg below, and shipped the FULL SUITE GREEN with the unscoped
        # sole-trigger claim published — measured 2026-08-22 in an isolated tree at the PRE-FIX
        # gates, 1190 passed. Emphasis capitals are this corpus's house style, so the caps form
        # is the LIKELY spelling and not an exotic one.
        assert not says(text, "only mechanical trigger"), (
...
def test_headship_rate_reads_a_single_age_and_a_hole_RAISES(tmp_path):
    ...
    curve = load_headship_rates(shape=census.HEADSHIP_CENTRAL_SHAPE)
...
def test_a_non_integer_or_out_of_range_age_key_is_refused_at_load(tmp_path):
    ...
    for bad in ("07", "3.0", "101"):
        ...
        with pytest.raises(LoaderError):
            load_headship_rates(data_dir=out, shape=census.HEADSHIP_CENTRAL_SHAPE)


def test_load_headship_rates_REQUIRES_the_shape_and_refuses_an_unknown_one():
    """The sweep's read path, and the selection site that no longer exists.

    `shape` is keyword-only and REQUIRED since the round-3 elegance audit (2026-08-22). The
    retired `shape=None` default resolved to the artifact's own `central_shape`: a second
    shape-selection site, reachable from any caller that stated nothing, pointed at a value
    outside `assumptions_hash`. A caller who states nothing now gets a TypeError — asserted
    here, because "the default is gone" is exactly the kind of claim a docstring can carry
    while the default is still there."""
    central = load_headship_rates(shape=census.HEADSHIP_CENTRAL_SHAPE)
    ...
    with pytest.raises(TypeError, match="shape"):
        load_headship_rates()                     # no shape stated => no curve served
    with pytest.raises(TypeError):
        load_headship_rates(census.HEADSHIP_CENTRAL_SHAPE)     # nor positionally


# --- ruling X2: THE LUMPED-RANGE AGGREGATE ------------------------------------------
#
# WHY THIS BLOCK EXISTS AT ALL, stated because its absence is what an audit found. Ruling X2
# added a SECOND accessor over this cube — `ownership_union_rates`, the household-weighted rate
# over a span of whole bands — and the run that added it shipped with ZERO tests naming either
# `ownership_union_rates` or `bands_spanning`. Two reviewers independently measured the
# consequence: reverting the immigrant leg to a point read inside the span left the ENTIRE suite
# green. The golden would catch the number moving, but `scripts/gen_golden.py` RE-RATIFIES
# whatever the code emits, so a gate that only fires through `test_golden.py` is not a gate —
# it is a record of the last run. These pins are the gate; the golden is the record.
#
# THE VALUES ARE ORACLED, NOT RESTATED. The union counts come from `_independent_band_counts`
# with a test-owned member list, i.e. from the CSV by this file's own reader, so a producer that
# tiled the span differently or weighted the bands by anything but their household counts reds
# here against arithmetic rather than against a literal someone copied out of a run log.
_UNION_SPAN = (25, 54)
_UNION_SPAN_MEMBERS = ("25 to 29 years", "30 to 34 years", "35 to 39 years",
                       "40 to 44 years", "45 to 49 years", "50 to 54 years")
# The served values, pinned as literals ON TOP of the oracle. The oracle proves the CONSTRUCTION;
# these prove the number a consumer actually receives has not drifted under a refactor that kept
# the construction and moved the routing.
_UNION_RATES = {
    Geography.MTL_RMR: 0.5119964493642796,
    Geography.QC_RMR: 0.5775769888793841,
    # The census-net residual. HORS_RMR's MODEL read is NOT this number — the join routes it to
    # `hors_aligned.aligned_ownership_union` (0.6901488081776652) — and the entry exists because
    # this function's strict join requires every geography to be present. See `pipeline.py`'s
    # `_ownership_reader`, which drops it so a join drift REFUSES instead of serving it.
    Geography.HORS_RMR: 0.6804520271369382,
}
# THE FORBIDDEN CONSTRUCTION, pinned as a NEGATIVE. An unweighted mean of the span's three band
# rates is not a rate, and it is the natural wrong implementation: it is off by -1.005 pp at
# MTL_RMR because the three bands carry materially different household counts. Nothing may serve
# it, at any geography.
_UNION_RATE_MEAN_MTL = 0.5019448407852299


def test_the_lumped_range_aggregate_is_formed_from_COUNTS_and_matches_an_oracle():
    """`ownership_union_rates` = summed owner counts / summed total counts over whole bands.

    THE ORACLE IS THE POINT. The span's counts are re-cut from the CSV by this file's own
    reader under a test-owned member list, so the leg below is an arithmetic identity a
    producer cannot satisfy by accident: a mean of band rates, a mis-tiled span, or a
    household-count weight taken from the wrong column all break it while still returning a
    plausible fraction.
    """
    union_counts = _independent_band_counts({"span": _UNION_SPAN_MEMBERS})
    served = ownership_union_rates(*_UNION_SPAN)

    for geo, source in _GEO_SOURCE.items():
        owner, total = union_counts[source]["span"]
        assert served[geo.value] == pytest.approx(owner / total, rel=1e-15), (
            f"{geo.value}: the {_UNION_SPAN} union is {served[geo.value]} against the "
            f"independently re-cut {owner}/{total} = {owner / total}")
    owner = union_counts[_PROVINCE]["span"][0] - sum(union_counts[c]["span"][0]
                                                    for c in _ALL_QC_CMAS)
    total = union_counts[_PROVINCE]["span"][1] - sum(union_counts[c]["span"][1]
                                                    for c in _ALL_QC_CMAS)
    assert served[Geography.HORS_RMR.value] == pytest.approx(owner / total, rel=1e-15), (
        "the HORS_RMR union is not the province NET of the six wholly-Québec CMAs")

    # THE SAME BORROWING AS THE BANDED DERIVATION, so the two surfaces cannot disagree about
    # what a geography IS. Read off `census._BORROWS_FROM` rather than re-listed: a second list
    # of borrowers here would be the drift vector, not the check.
    for geo, parent in census._BORROWS_FROM.items():
        assert served[geo.value] == served[parent.value], (
            f"{geo.value} no longer borrows {parent.value}'s union with the rest of its curve")
    assert set(served) == {g.value for g in Geography}, (
        f"the union's strict join emitted {sorted(served)}")


def test_the_lumped_range_aggregate_serves_the_PINNED_values_and_never_a_mean_of_RATES():
    """The VALUE pin plus its negative twin — what a consumer receives, and what it may not.

    THE NEGATIVE LEG IS THE ONE WITH TEETH. Every value here is a plausible ownership fraction,
    so a wrong construction does not announce itself; it has to be named. Two are named: no
    geography may ever be served the unweighted MEAN of the span's band rates, and no geography
    may ever be served any single band's rate from inside the span. Those are exactly the two
    constructions ruling X2 forbids, and until this test existed neither was checked.
    """
    served = ownership_union_rates(*_UNION_SPAN)
    for geo, expected in _UNION_RATES.items():
        assert served[geo.value] == pytest.approx(expected, rel=1e-15), geo.value

    lo, hi = _UNION_SPAN
    inside = [label for label, _lo, _hi, _m in bands_spanning(lo, hi)]
    assert len(inside) == 3, f"the {lo}-{hi} span is no longer three model bands: {inside}"
    rates = load_ownership_rates()
    for geo in Geography:
        band_rates = [rates[geo.value][label] for label in inside]
        assert served[geo.value] not in band_rates, (
            f"{geo.value}'s {lo}-{hi} union equals one of its own band rates {band_rates} — a "
            "point read inside the span is being served as the span's aggregate (ruling X2)")
        mean_of_rates = sum(band_rates) / len(band_rates)
        assert served[geo.value] != pytest.approx(mean_of_rates, rel=1e-9), (
            f"{geo.value} is served the unweighted mean of its band rates ({mean_of_rates}) — "
            "a mean of rates is not a rate; the union is formed from counts and divided once")
    assert sum(rates[Geography.MTL_RMR.value][label] for label in inside) / 3 == pytest.approx(
        _UNION_RATE_MEAN_MTL, rel=1e-15), (
        "the forbidden construction's own value moved — re-derive the -1.005 pp figure "
        "`census.py` and `constants.MODEL_CHOICE_PROVENANCE` both restate")
    # THE HARDCODED SET-MEMBERSHIP TWIN WAS DELETED HERE (2026-08-21 audit):
    #   assert _UNION_RATE_MEAN_MTL not in set(served.values())
    # Entailed by the `rel=1e-15` value pins at the top of this test — the CLOSEST served union
    # sits 1.963e-02 relative from the literal, about 13 orders of magnitude outside that
    # tolerance — so it could not fail while those passed. The
    # LIVE-COMPUTED legs in the loop above (`served[geo] not in band_rates` and
    # `served[geo] != approx(mean_of_rates)`) are the real gates: they re-derive both forbidden
    # constructions per geography rather than comparing against MTL_RMR's frozen number, and
    # they survive a refresh that would move every pin in this file. The literal keeps its one
    # real job — the assert directly above, which pins the forbidden construction's own value so
    # the -1.005 pp figure `census.py` and `constants.MODEL_CHOICE_PROVENANCE` both restate has
    # something under it.


def test_bands_spanning_REFUSES_every_range_that_is_not_an_exact_union_of_whole_bands():
    """The refusal branches, which were hand-verified when they were written and gated by
    nothing — so nothing checked that they keep refusing.

    A partly-covered band is the silent failure this accessor exists to make loud: the result
    would be a plausible fraction measured over a population nobody asked for. It is also the
    guard that makes the accessor survive the NEXT lattice change, so its refusals matter more
    than its successes.
    """
    lo, hi = _UNION_SPAN
    assert [b[0] for b in bands_spanning(lo, hi)] == ["25-34", "35-44", "45-54"]
    assert [b[0] for b in bands_spanning(25, 200)] == list(_SPEC_AGE_BANDS)

    with pytest.raises(LoaderError, match="EXACT union"):
        bands_spanning(25, 50)              # right edge inside 45-54
    with pytest.raises(LoaderError, match="EXACT union"):
        bands_spanning(30, 54)              # left edge inside 25-34
    with pytest.raises(LoaderError, match="EXACT union"):
        bands_spanning(20, 54)              # below the lattice floor
    with pytest.raises(LoaderError, match="no modeled ownership band"):
        bands_spanning(26, 33)              # inside ONE band, tiling nothing
    with pytest.raises(LoaderError, match="no modeled ownership band"):
        bands_spanning(15, 24)              # entirely below the floor


def test_the_lumped_range_aggregate_verifies_the_extract_PIN_on_its_own_read(tmp_path):
    """It reads the pinned extract through the one public seam, so a corrupted extract REFUSES
    here rather than being aggregated into a plausible fraction.

    This is also the leg that makes `pipeline._verify_pinned`'s docstring true: since ruling X2
    the census extract IS opened on a runtime path, so its pin is checked twice rather than only
    at envelope time.
    """
    shutil.copytree(DATA_DIR, tmp_path / "d")
    target = tmp_path / "d" / CENSUS_EXTRACT
    target.write_bytes(target.read_bytes().replace(b"Number of private households",
                                                   b"Number of private householdz", 1))
    with pytest.raises(LoaderError, match="sha256|pin"):
        ownership_union_rates(*_UNION_SPAN, data_dir=tmp_path / "d")
