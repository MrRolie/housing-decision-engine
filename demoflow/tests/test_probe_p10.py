...
def _wire(probe, tmp_path: Path, **overrides):
    ...
    for name, attribute in (("spec", "_spec_s6"), ("qfe", "_qfe_record"), ("p9", "_p9_note"),
...
def test_p10_the_rho_bands_are_the_models_own_lattice(probe):
    """Measured in `census._AGE_BAND_SPEC`'s bands, not in a proxy's: the two cubes publish
    different age granularities of the SAME partition, and the band EDGES are the model's."""
    from demoflow.loaders import census

    assert probe.RHO_BAND_ORDER == tuple(label for label, _lo, _hi, _m in census._AGE_BAND_SPEC)
    for label, lo, hi, _members in census._AGE_BAND_SPEC:
        for spec in (probe.RHO_BANDS_CMA, probe.RHO_BANDS_CD):
            edges = [int(re.match(r"(\d+)", name).group(1)) for name in spec[label]]
            assert min(edges) == lo, (label, spec[label])
            assert max(edges) <= hi, (label, spec[label])
...
def test_p10_floor_guards_are_load_bearing(offline, mutation, boundary):
    ...
    elif mutation == "qfe-figure-perturbed":
        real = mod._qfe_record()
        assert "1.0242" in real
        extra["qfe"] = lambda: real.replace("1.0242", "1.0252")
...
def test_p10_the_separability_claim_is_measured_and_not_only_quoted(offline, note):
    ...
    assert f"{flow['hors_mean']:,.0f}" in note and f"{flow['gatineau_mean']:,.0f}" in note
