...
# The record's OPTIONAL set, read from the module rather than restated: it grew at spec
# amendment #21 (`freshness_years`, `source_kind`) and a literal `{"reason"}` here would have to
# be edited every time the emitted row gains an optional member — a maintenance tax whose whole
# effect is to make the allowlist checks below say something narrower than the contract.
from demoflow.output.tripwires import _OPTIONAL
...
def test_closed_plan_year_crosses_high_on_the_measured_provincial_sum(tmp_path):
    ...
    assert set(rec) <= set(TRIPWIRE_RECORD_REQUIRED) | _OPTIONAL
    assert exit_code([ev.result]) != 0
...
def test_run_log_detail_never_reaches_the_record(tmp_path):
    ...
    assert set(tripwire_record(ev.result)) <= set(TRIPWIRE_RECORD_REQUIRED) | _OPTIONAL
...
def test_an_intact_partial_year_reports_the_member_gap_without_claiming_its_cause(tmp_path):
    ...
    # `says` on the two FORBIDS (run 48): the log's cause language is prose, and this pair is
    # asserted in BOTH directions a dozen lines apart — the withheld case here, the earned case
    # below — so a casing change would silently satisfy one of them whichever way it went.
    assert not says(ev.log, "cannot explain")                           # the cause, withheld
    assert not says(ev.log, "publication-calendar gap")   # the shipped phrasing said exactly
    ...
    assert "cannot explain" in log and not says(log, "cause is NOT claimed")
