...
from actuarial.compat import active_mortality, set_active_mortality
...
# no digest (see the `source.directory` entry in `uv.lock`), and
...
def test_the_basis_digest_reaches_no_private_engine_surface():
    ...
    engine_private = [(mod, name) for mod, name in imported
                      if (mod or "") == "actuarial.compat" and name.startswith("_")]
