...
def test_p9_the_closure_is_scoped_and_the_residual_is_named(note):
    ...
    # `says` (run 48): an absence claim re-typed "NO SUCH SOURCE EXISTS" — the emphasis form
    # a probe note is MORE likely to use for a verdict than less — walked past the bare
    # membership test, while every required residual leg above stayed satisfied.
    assert not says(note, "no such source exists"), (
