...
def test_band_labels_partition_75_plus_and_agree_with_the_ownership_bands():
    """A label shared with census.py must denote the SAME domain — the miscompose is a shared
    SPELLING over a different range, never the sharing itself.

    RESHAPED AT OPERATOR RULING W (2026-08-20), and the reshape is the finding. The gate was
    written when census.py published a single `75+` band meaning 75..200 while this loader
    published `75-84` and `85+`: it asserted the label sets were DISJOINT, on the ground that
    the plan's original `"75+"` label for the 75-84 band would have collided. Ruling W split
    census's tail into exactly `75-84` (75..84) and `85+` (85..200) — the same two labels over
    the same two ranges as here — so disjointness is now FALSE, and it is false because the two
    lattices CONVERGED rather than because either drifted.

    Asserting disjointness would now forbid agreement, which inverts the invariant. What the
    old gate was really protecting is kept and stated directly: every label these two loaders
    share must carry an IDENTICAL (lo, hi). The bare `75+` spelling is separately asserted
    ABSENT here, because it still lives in census's LEGACY headship labels where it means
    75..100 — a third range under the same word, and the original collision this gate names.
    """
    ...
    for label, lo, hi, member in spec:
        ...
        prev_hi = hi
    mine = {label: (lo, hi) for label, lo, hi, _ in spec}
    # `census._HEADSHIP_BANDS` is GONE (operator ruling V): headship is age-resolved and its
    # six LEGACY band labels survive only as provenance, so the surface to check is the
    # ownership lattice plus that legacy label set — the same labels, read from the specs that
    # still publish them.
    ownership = {label: (lo, hi) for label, lo, hi in census._AGE_BANDS}
    legacy = {label: (lo, hi) for label, lo, hi in census._HEADSHIP_LEGACY_BAND_SPEC}

    # (1) THE INVARIANT: a shared label is a shared DOMAIN. Since ruling W both of this
    # loader's labels are shared with the ownership lattice, and both must match it exactly.
    for label, (lo, hi) in mine.items():
        if label in ownership:
            assert ownership[label] == (lo, hi), (
                f"band {label!r} means {(lo, hi)} here and {ownership[label]} in census.py's "
                "ownership lattice — one spelling over two domains is a silent miscompose")
    assert set(mine) <= set(ownership), (
        f"a band label here is absent from the ownership lattice entirely: "
        f"{sorted(set(mine) - set(ownership))} — the two surfaces multiply the same population "
        "in the cohort initialization, so an unmatched tail label is a join with no partner")
    # THE "NON-VACUITY" ASSERT THAT STOOD HERE IS DELETED (audit, 2026-08-21). It read
    # `set(mine) & set(ownership) == set(mine) and mine` — and `A & B == A` IS `A <= B`, the
    # assertion immediately above, so the only part of it that could fail independently was
    # `mine` being non-empty. That is pinned harder three lines up, where the label tuple is
    # held EQUAL to `("75-84", "85+")`. Non-vacuity of the agreement loop therefore follows from
    # two checks that CAN fail — the label pin (exactly two labels) and the containment pin
    # (both are ownership labels) — which is where it belongs. A restatement of a neighbouring
    # assert reads like a second, independent guard and is not one.

    # (2) THE ORIGINAL COLLISION, still asserted absent. The bare `75+` spelling survives in
    # census's LEGACY headship labels where it means 75..100 — a THIRD range under one word —
    # so the plan's original `"75+"` label for the 75-84 band would still be a miscompose.
    assert "75+" in legacy and "75+" not in mine, (
        "the plan's `75+` label for the 75-84 band would collide with the legacy headship "
        f"band's {legacy.get('75+')} domain")
    assert "75+" not in ownership, (
        "the ownership lattice carries a bare `75+` again — ruling W split it into 75-84 and "
        "85+, and this gate's agreement leg above assumes that split")
