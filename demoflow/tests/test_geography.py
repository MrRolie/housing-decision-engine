...
def test_the_modeled_domain_is_DERIVED_from_the_loader_registry_not_typed_out():
    """`MODELED_GEOGRAPHIES` is the geography domain the rankings artifact must account for
    (spec §8: "the rankings cover the remaining members"), and it is the UNION of the
    per-workbook expected sets the loaders already enforce through `require_all_geographies`
    — never a second literal list. A typed list is the stale-constant class this repo keeps
    re-finding: it would go quietly wrong the day a workbook's expected set changes.

    THE TWO DECLARATIONS ARE COMPARED HERE, which is the half a derivation cannot do for
    itself. `set(Geography)` and the registry union are independent statements about the same
    domain, so the day they disagree is the day either an enum member exists that no committed
    workbook is required to supply, or a workbook is required to supply a member the emitters
    cannot name. Measured n=8 against the two POPULATION workbooks the run concatenates,
    imported from the pipeline so the count is read off the run's own source list."""
    from demoflow.geography import MODELED_GEOGRAPHIES, WORKBOOK_GEOGRAPHIES
    from demoflow.pipeline import POP_WORKBOOKS

    assert MODELED_GEOGRAPHIES == frozenset(Geography)
    assert len(MODELED_GEOGRAPHIES) == 8
    assert MODELED_GEOGRAPHIES == frozenset().union(
        *(WORKBOOK_GEOGRAPHIES[name] for name in POP_WORKBOOKS))
    assert all(isinstance(geo, Geography) for geo in MODELED_GEOGRAPHIES)
