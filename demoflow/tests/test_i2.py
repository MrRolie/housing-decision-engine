...
def test_an_unknown_provenance_raises_at_construction(field):
    kwargs = dict(geography=Geography.MTL_RMR, immigrant_headship=0.5, ownership_ratio=1.0,
                  headship_provenance="cited", ratio_provenance="cited", pooled=_POOLED,
...
def test_the_single_pair_flag_is_superseded():
    ...
    assert {f.name for f in dataclasses.fields(ImmigrantInputs)} == {
        ...
        "headship_provenance", "ratio_provenance", "source", "pooled"}


# A VALID pooled-ownership block for the construction-guard tests below (amendment #24(A) made
# `pooled` a required field). MTL_RMR's own counts, so every guard test constructs a row whose
# ONLY defect is the one it is exercising — a row that also carried bad counts would raise from
# `PooledOwnership` and green the test on the wrong guard.
_POOLED = ii.PooledOwnership(non_immigrant=(1252635, 724630), immigrant=(511070, 266470),
                             non_permanent_resident=(72000, 7135))
    ...
    for headship in (0.154, 0.10):
        with pytest.raises(LoaderError, match="living-alone floor"):
            ImmigrantInputs(geography=Geography.MTL_RMR, immigrant_headship=headship,
                            ...
                            ratio_provenance="cited", pooled=_POOLED,
                            source="a source string long enough to be a citation")
...
# ================================================== the POOLED counts and B (amendment #24(A))
#
# The counts `PooledOwnership` carries are the same class of literal as the ruled headship/ratio
# pair above and get the same treatment: coupled to P8's own committed tables ON THE DIGITS, so a
# regenerated note whose counts move REDS this table instead of leaving it a decorative copy of a
# measurement nobody re-checked. The rows are located by (geography, MEMBER NAME) inside P8's
# per-member cross-tabs — §2a at CMA grain and §3 at census-division grain — which is where the
# three mutually exclusive population groups are published.

# P8's own column order in those tables: persons | maintainers | owner-maintainers | ... The two
# this table consumes are the 2nd and 3rd.
_P8_MEMBER_ROW = (r"^\|\s*{geo}\s*\|\s*{member}\s*\|\s*([\d,]+)\s*\|\s*([\d,]+)\s*\|"
                  r"\s*([\d,]+)\s*\|")
# PooledOwnership field name -> the member name P8 publishes it under. CLOSED: the assertion
# below is set-equality over the dataclass's own fields, so a fourth group cannot be added to the
# class without being given a member here.
_P8_MEMBERS = {"non_immigrant": "Non-immigrants",
               "immigrant": "Total immigrants",
               "non_permanent_resident": "Non-permanent residents"}


def _p8_counts(text: str, geo: Geography, member: str) -> tuple[int, int]:
    """`(maintainers, owner-maintainers)` for one (geography, member) row of P8's cross-tabs."""
    pattern = _P8_MEMBER_ROW.format(geo=re.escape(geo.value), member=re.escape(member))
    found = re.findall(pattern, text, re.MULTILINE)
    assert len(found) == 1, (
        f"P8 publishes {len(found)} rows for ({geo.value}, {member!r}) in the shape this gate "
        f"reads ({pattern!r}) — absence discipline: a parser that finds none makes the "
        f"comparison below vacuous, and two rows means the wrong one could be read")
    persons, maintainers, owners = (int(x.replace(",", "")) for x in found[0])
    assert persons > maintainers > owners > 0, (
        f"({geo.value}, {member!r}): P8's row reads persons={persons}, maintainers="
        f"{maintainers}, owners={owners} — the columns are not in the order this gate assumes")
    return maintainers, owners


def test_the_pooled_counts_are_P8s_OWN_DIGITS_at_every_measured_geography():
    """Amendment #24(A)'s operand, coupled to the measurement rather than transcribed from it.

    THE MEMBER MAP IS ASSERTED TOTAL over `PooledOwnership`'s fields, which is the half a
    row-by-row comparison cannot give: a fourth population group added to the class with no P8
    member behind it would otherwise ride into `B`'s denominator uncoupled to anything.

    HORS_RMR's ROW IS THE PRE-#13 TERRITORY'S AND THAT IS THE POINT OF READING IT FROM P8. §2a
    publishes the per-member decomposition only at the province-net-of-six residual — the aligned
    correction was measured at three members and deliberately not extended — so the counts this
    table carries are that territory's, and this coupling is what makes that visible rather than
    a silent choice. `immigrant_inputs.PooledOwnership` declares the residual in prose."""
    text = _note_text()
    assert set(_P8_MEMBERS) == {f.name for f in dataclasses.fields(ii.PooledOwnership)}, (
        f"`PooledOwnership`'s fields are "
        f"{sorted(f.name for f in dataclasses.fields(ii.PooledOwnership))} and this gate maps "
        f"{sorted(_P8_MEMBERS)} to P8 members — every group in `B`'s denominator must be coupled")
    for geo in MEASURED:
        pooled = resolve_immigrant_inputs(geo).pooled
        for field, member in _P8_MEMBERS.items():
            assert getattr(pooled, field) == _p8_counts(text, geo, member), (
                f"{geo.value}/{field}: the table carries {getattr(pooled, field)} and P8's "
                f"{member!r} row publishes {_p8_counts(text, geo, member)}")


def test_B_is_COMPUTED_from_those_counts_and_the_proxies_DERIVE_it():
    """`B = p_all / p_nonimm`, recomputed here from P8's digits by an independent expression.

    The point is that NOTHING in the module types a value of `B`: the property is arithmetic over
    counts that are themselves coupled to the note above, so a re-measured note moves `B`, moves
    every emitted ED and reds the golden — the chain amendment #24(A) rules and the reason it
    refused a declared limit in favour of a fix.

    THE SIGN OF THE CORRECTION IS ASSERTED PER GEOGRAPHY, not in aggregate: exactly one modeled
    geography carries `B > 1` (`LAVAL_RA13`, where immigrants and non-permanent residents own at
    rates that pull the pooled rate ABOVE the non-immigrant one), so it is the single place the
    shipped leg was OVER-stated. An aggregate claim would be satisfied by the wrong geography."""
    text = _note_text()
    for geo in MEASURED:
        counts = {field: _p8_counts(text, geo, member)
                  for field, member in _P8_MEMBERS.items()}
        maintainers = sum(m for m, _o in counts.values())
        owners = sum(o for _m, o in counts.values())
        nm, no = counts["non_immigrant"]
        expected = (owners / maintainers) / (no / nm)
        assert resolve_immigrant_inputs(geo).pooled.pooled_denominator_bias == expected, (
            f"{geo.value}: the served bias is "
            f"{resolve_immigrant_inputs(geo).pooled.pooled_denominator_bias!r} and P8's counts "
            f"give {expected!r}")

    above_one = {g.value for g in Geography
                 if resolve_immigrant_inputs(g).pooled.pooled_denominator_bias > 1.0}
    assert above_one == {"LAVAL_RA13"}, (
        f"the geographies whose pooled rate EXCEEDS their non-immigrant rate are {above_one}; "
        "amendment #24(A) measures exactly LAVAL_RA13 there, and that is the one row whose "
        "shipped immigrant leg was over-stated rather than under-stated")

    # ...and the proxies DERIVE their parent's, never re-type it — the rule the ruled pair follows
    for proxy, parent in ii._PARENT_CMA.items():
        assert (resolve_immigrant_inputs(proxy).pooled
                is resolve_immigrant_inputs(parent).pooled), (
            f"{proxy.value} carries its own pooled block rather than the {parent.value} row's — a "
            "borrowed row's whole immigrant input is the parent's, so a re-typed copy would let "
            "the parent's counts move without moving the proxy's bias")


def test_pooled_counts_that_are_not_counts_or_not_a_subset_RAISE_at_construction():
    """The two ways a hand edit can make these counts lie, both refused at import.

    An INVERTED pair (more owners than maintainers) reads downstream as an ownership propensity
    above 1 — a plausible-looking float that would divide into `B` and bias every ED with nothing
    to see. A FLOAT where a count belongs is the transcription of a rate somebody already divided,
    which is the defect the whole DERIVATION-NEVER-TRANSCRIPTION rule exists to refuse."""
    with pytest.raises(LoaderError, match="subset of them"):
        ii.PooledOwnership(non_immigrant=(100, 200), immigrant=(10, 5),
                           non_permanent_resident=(10, 5))
    with pytest.raises(LoaderError, match="publishes COUNTS"):
        ii.PooledOwnership(non_immigrant=(100.0, 50.0), immigrant=(10, 5),
                           non_permanent_resident=(10, 5))
    with pytest.raises(LoaderError, match="positive"):
        ii.PooledOwnership(non_immigrant=(0, 0), immigrant=(10, 5),
                           non_permanent_resident=(10, 5))
