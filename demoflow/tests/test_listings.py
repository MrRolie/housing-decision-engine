...
literals whose anchors live in `CONSTANTS` — which the round-3 audit (2026-08-22) brought INSIDE
the hash payload, so the discriminator is no longer coverage but ROLE: a band that only the
robustness sweep reads is pinned against its anchor test-side, while these three are central
values the ED path multiplies directly and must be bound read-through. Same single-source rule,
different enforcement, because the failure modes differ.
"""
...
from demoflow.cohort.listings import (
    market_listings, PHI_VOLUNTARY, ESTATE_EVENTUAL_FRACTION, ESTATE_LAG_YEARS,
)
...
# THE PLAN'S `test_unknown_cause_raises` IS RETIRED WITH ITS SUBJECT (round-3 elegance audit,
# 2026-08-22). It asserted `phi_market("bequest_of_dragons")` raises; `phi_market` is deleted —
# zero non-test callers, and it read this module's FROZEN constants rather than the caller's
# parameters, which is the run-32 CRITICAL's own mechanism. The exit-cause VOCABULARY is still
# refused, and now only where the causes are actually mapped:
# `tests/test_pipeline.py::test_exit_causes_map_explicitly_to_listing_causes` reds on
# `pipeline._split_exits({"estate": 1.0, "moved_to_mars": 2.0})`.
...
def test_the_convolution_moves_the_YEAR_and_conserves_the_MAGNITUDE():
    """Spec §7's supply term is `S = Σ_cause exits(cause)·φ(cause), estate lagged L`, and the lag
    is a rider on the estate leg ALONE. So the fractioned total over a decade is invariant to the
    lag while the year it lands in is not — which is what makes `market_listings` the only honest
    entry point, and is why the per-cause fraction accessor that served the φ half without the
    convolution was deleted rather than documented (round-3 elegance audit, 2026-08-22).

    Asserted against the two module CONSTANTS rather than against an accessor: the φ values are
    what the equation names, and reading them off the surface the pipeline passes them through is
    the check that survives the deletion."""
    unlagged_total = 40.0 * PHI_VOLUNTARY + 100.0 * ESTATE_EVENTUAL_FRACTION
    ...
    # ...and the estate leg landed at 2040 + the central lag, carrying its own fraction alone.
    assert convolved[2040 + ESTATE_LAG_YEARS] == pytest.approx(100.0 * ESTATE_EVENTUAL_FRACTION)
