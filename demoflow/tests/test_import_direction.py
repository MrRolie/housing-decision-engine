...
**actuarial-system's** distribution (uv path dep), never to anything in this repo. So the
source-grep contract forbids `hde` specifically, and the compat-import test PINS THE
PROVENANCE of `actuarial` via `packages_distributions()` rather than merely asserting the
import succeeds — a bare success check would silently start passing against a same-named
local package the day someone adds one.
"""
