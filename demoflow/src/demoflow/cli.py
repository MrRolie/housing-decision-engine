...
five workbooks, evaluated the excess-demand grid once for every leg `_sweep_legs` declares (the
central run, which one sweep leg reuses, plus the rest of the rank-stability sweep), and WROTE
`rankings.json` into whatever directory the operator was standing in. `run` emits; `tripwires`
evaluates. They are routed to different functions and only one of them can write anything.
...
not have to guess, so it is stated here, in `--help`, and in the contract tests:
  ...
  * `demoflow run` exits on EMISSION success — all three documents built, every contract validated,
    all files written. It deliberately does NOT adopt the tripwire verdict: on the committed
...
_EPILOG = """exit codes (they answer DIFFERENT questions):
  run        0 on EMISSION success -- all three documents built, every contract
             validated, all files written. NOT the tripwire verdict: a run whose indicators are all UNKNOWN
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run = sub.add_parser(
        "run", help="emit rankings + tripwire + scenario-prior artifacts (exits 0 on emission "
                    "success)",
        description="Run the pipeline and emit rankings.json + tripwire_baseline.json + "
                    "scenario_prior.json. Exits 0 on EMISSION success -- not on the "
                    "tripwire verdict, which is printed and available from "
                    "`demoflow tripwires`.")
    run.add_argument("--out", type=Path, default=Path("artifacts"),
                     help="directory to write all three artifacts into (default: ./artifacts)")
...
def _print_identity(assumptions_hash: str, vintage: dict) -> None:
    ...
    AND THE PARAGRAPH ABOVE WAS FALSE OF THIS FUNCTION UNTIL 2026-08-22, in exactly the way it
    warns about. It named two fields per entry and `committed_sha256` — spec amendment #20(C)(1)'s
    new member — was neither, so two runs over DIFFERENT committed bytes of
    `census_tenure_age_98100231.csv` rendered IDENTICALLY here (measured). That is the ONE key
    `artifacts/README.md` tells a consumer to verify against, on the only identity surface the CLI
    has. So the rendering is now TOTAL: every key the entry holds, sorted, so a member added to
    the envelope cannot be dropped by a printer that never heard of it. Sorted rather than
    hand-ordered for that reason — a fixed field list is what went stale, and a prettier order
    would have to be edited by the same author who forgot the field.
    """
    ...
    for name in sorted(sources):
        entry = sources[name]
        fields = " ".join(f"{key}={entry[key]}" for key in sorted(entry))
        print(f"  {name} {fields}")
