"""Persons->households conversion (spec §5 AS AMENDED by the steering-ruling-A balance-gate amendment, 2026-07-25).
...
aggregate only. ITS CALL SITE IS LIVE: `pipeline._standing_stock` sums `age >= 75` per sex,
hands exactly that aggregate to `initialize_households` through `pipeline._household_stock`, and
that function calls this gate on the result whenever it is given a `direction_ctx` — which only
`_standing_stock` passes. `_band_entry_stock` deliberately does NOT: it hands a SINGLE age-75
cohort, and a direction gate applied per band is a different overreach in the family steering
ruling A retired the per-band MAGNITUDE gate from.

THE CARRY THAT STOOD HERE UNTIL 2026-08-22 SAID THIS CALLER "does not exist as code yet", AND
TASK 29 HAD LANDED — under a renamed function (`_init_stock` -> `_standing_stock`), with
`HouseholdInit` returning exactly `coupled_m`/`coupled_f` and `_household_stock` DISCARDING
both. So the gate had ZERO production callers, two committed contracts said it fired (this
carry and `loaders/living_arrangement.py`'s gate list), and a round-3 audit measured what
shipped without it: transposing the M/F living-arrangement reads inside `_household_stock`
computes the FULL ED grid with NO refusal — LAVAL_RA13 flips sign, the negative-ED count drops
3 -> 2, MTL_RMR moves ~40x the tightest rank-deciding gap — while this gate would have raised at
all eight geographies. It is ONE-DIRECTIONAL (`coupled_f <= coupled_m` returns early), so it
does NOT see an ISQ sex-COLUMN transposition, which moves the same count and leaves M > F.

Same shape `living_arrangement._assert_coupled_direction` uses for the SOURCE-side version of
this check (different error class there on purpose: that one detects a source/junction
transposition and raises `LoaderError`; this one gates MODELLED counts and raises
`CalibrationError`).
