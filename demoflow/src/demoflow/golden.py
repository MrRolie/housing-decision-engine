...
# The two documents the golden covers, named once. `generate_golden`'s caller BINDS this to
# what the run actually emitted (`tests/test_golden.py` asserts the equality), so a third
# document added to `run_pipeline` cannot slip past the golden's strict-JSON scan by being
# absent from a list nobody re-checked.
GOLDEN_ARTIFACTS = ("rankings.json", "tripwire_baseline.json")
