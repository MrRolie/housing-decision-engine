...
THREE NUMBERS PER GEOGRAPHY SINCE AMENDMENT #24(A), not two: the pooled ownership COUNTS
ride each row as well (`PooledOwnership`), because the ratio's non-immigrant denominator does
not match the pooled denominator of the propensity CURVE it multiplies. `B` is computed from
those counts per geography and the immigrant leg divides by it — see `PooledOwnership` for what
that corrects and `pipeline._ed_series` for the one place it is applied.
...
class PooledOwnership:
    """The immigrant cube's OWN counts at one geography, as `(maintainers, owner-maintainers)`
    per population-characteristics member — the operand of amendment #24(A)'s conversion.

    WHY THIS EXISTS AT ALL. The age-banded ownership cube the model reads its propensity CURVE
    from (98-10-0231-01) carries NO immigrant dimension, so the rate it serves is
    `owner maintainers / ALL maintainers` — immigrants, non-immigrants AND non-permanent
    residents. The `ownership_ratio` above is denominated on NON-IMMIGRANTS alone. Multiplying
    the first by the second does not recover immigrant ownership: it emits
    `p_imm_true x B`, with `B = p_all / p_nonimm`. These counts are what `B` is computed FROM,
    per geography, so the correction uses the SAME cube that supplies the ratio and no scalar is
    typed anywhere.

    THE THREE MEMBERS ARE THE ONES THAT PARTITION THE UNIVERSE, and that is why they are NOT the
    member the ratio uses. The ratio is measured on `Before 2016` — a SUBSET of immigrants,
    ruled for the reasons §6 records. `B`'s denominator is the POOLED one, so its members must
    ADD UP to the pooled universe: non-immigrants + ALL immigrants + non-permanent residents.
    Corroborated across cubes at MTL_RMR: these three maintainer counts sum to 1,835,705, which
    is the age-banded cube's own MTL total private households, difference 0 — so the pooled
    operand's universe is provably all three groups and not a subset of them.

    HORS_RMR's COUNTS ARE THE PRE-#13 TERRITORY'S, and the mismatch is DECLARED here rather
    than papered over. The ruled `ownership_ratio` at that row is measured over the
    operand-aligned territory (province net of six CMAs, further net of Ottawa-Gatineau's
    Québec-side subdivisions), while `98-10-0621-01` publishes no Québec-part member at CMA
    grain — so probe P8 measured the aligned correction at the THREE members the ruled pair
    needs and explicitly did NOT extend it to `Total immigrants` or `Non-permanent residents`.
    The counts below are therefore the province-net-of-six residual's, which is the only
    per-member decomposition this tree HAS at that geography. `B` is a ratio of two rates over
    one territory, so the residual is a territory mismatch in the CORRECTION FACTOR, not in the
    rate: aligning it needs the same live pull that produced the aligned pair, which is where
    the number would have to come from and not from arithmetic here.

    NON-PERMANENT RESIDENTS ARE CARRIED SEPARATELY AND THAT IS LOAD-BEARING, not tidiness. A
    two-group correction predicts `B` at MTL_RMR as 0.97252 against the measured 0.940022,
    because the pooled denominator carries a THIRD group at weight 0.0392 and propensity ratio
    0.1713. The real bias is roughly DOUBLE the two-group figure, which is the whole argument for
    deriving `B` from counts rather than from a stated immigrant weight.
    """

    non_immigrant: tuple[int, int]
    immigrant: tuple[int, int]
    non_permanent_resident: tuple[int, int]

    def __post_init__(self) -> None:
        # RANGED OVER THE DATACLASS'S OWN FIELDS, never a hand-listed triple: a fourth population
        # group added above would otherwise ride into `B`'s denominator ungated, which is the one
        # way a count here can be wrong and stay invisible.
        for name, pair in ((field.name, getattr(self, field.name))
                           for field in fields(self)):
            maintainers, owners = pair
            if not (isinstance(maintainers, int) and isinstance(owners, int)):
                raise LoaderError(f"pooled ownership counts for {name} are {pair!r} — the cube "
                                  f"publishes COUNTS, and a float here is a transcription of a "
                                  f"rate somebody already divided")
            if maintainers <= 0 or owners < 0 or owners > maintainers:
                raise LoaderError(
                    f"pooled ownership counts for {name} are {pair!r}: maintainers must be "
                    f"positive and owner-maintainers a subset of them — an inverted pair reads "
                    f"as a plausible propensity above 1 and nothing downstream would notice")

    @property
    def _groups(self) -> tuple[tuple[int, int], ...]:
        """Every member's `(maintainers, owner-maintainers)` pair — off the field list, so the
        pooled rate below sums over whatever groups the class declares."""
        return tuple(getattr(self, field.name) for field in fields(self))

    @property
    def all_maintainer_rate(self) -> float:
        """`p_all` — owner maintainers over ALL maintainers, summed over the three members.

        THE SUM, NEVER THE CUBE'S OWN `Total` ROW, and the reason is availability rather than
        preference: the census-division sibling publishes these three members at RA06 and
        RA13 and this run reads no `Total` row there, so the sum is the ONE construction
        available at every modeled geography. Where both exist they agree to a single rounding
        step (MTL_RMR: 998,235 summed against 998,230 published), because StatCan rounds each
        published cell to the nearest 5 independently."""
        return (sum(owners for _m, owners in self._groups)
                / sum(maintainers for maintainers, _o in self._groups))

    @property
    def non_immigrant_rate(self) -> float:
        """`p_nonimm` — the rate the `ownership_ratio`'s denominator is measured on."""
        maintainers, owners = self.non_immigrant
        return owners / maintainers

    @property
    def pooled_denominator_bias(self) -> float:
        """`B = p_all / p_nonimm` — the factor the shipped immigrant leg was multiplied by.

        COMPUTED, NEVER TYPED (amendment #24(A) rules exactly this). Measured on the committed
        counts: MTL_RMR 0.940022, QC_RMR 0.970180, MTL_ISLAND_RA06 0.939637, HORS_RMR 0.990212
        and LAVAL_RA13 1.016742 — the ONE geography above 1, i.e. the one place the shipped
        immigrant leg was OVER-stated rather than under-stated. Those figures are a reader's
        cross-check on this arithmetic and are deliberately absent from the code: a typed
        constant here is the defect class the surrounding module exists to refuse."""
        return self.all_maintainer_rate / self.non_immigrant_rate
...
_MEASURED = {
    Geography.MTL_RMR: ImmigrantInputs(
        ...
        pooled=PooledOwnership(non_immigrant=(1252635, 724630), immigrant=(511070, 266470),
    ...
    Geography.QC_RMR: ImmigrantInputs(
        ...
        headship_provenance="cited", ratio_provenance="cited",
        pooled=PooledOwnership(non_immigrant=(355550, 213555), immigrant=(25400, 12045),
                               non_permanent_resident=(7005, 470)),
    ...
    Geography.HORS_RMR: ImmigrantInputs(
        ...
        pooled=PooledOwnership(non_immigrant=(1165570, 813230), immigrant=(49840, 31240),
                               non_permanent_resident=(8195, 895)),
    ...
    Geography.MTL_ISLAND_RA06: ImmigrantInputs(
        ...
        headship_provenance="cited", ratio_provenance="cited",
        pooled=PooledOwnership(non_immigrant=(502530, 211560), immigrant=(343815, 143185),
                               non_permanent_resident=(64020, 5375)),
    ...
    Geography.LAVAL_RA13: ImmigrantInputs(
        ...
        headship_provenance="cited", ratio_provenance="cited",
        pooled=PooledOwnership(non_immigrant=(105975, 69370), immigrant=(62065, 43345),
                               non_permanent_resident=(1745, 285)),
...
def _borrowed(member: Geography, parent: Geography) -> ImmigrantInputs:
    ...
    return ImmigrantInputs(
        ...
        pooled=row.pooled,                              # DERIVED — one universe per row
...
def resolved_selection() -> dict[str, dict[str, object]]:
    ...
    return {geography.value: {"immigrant_headship": row.immigrant_headship,
                              ...
                              "pooled_non_immigrant": list(row.pooled.non_immigrant),
                              "pooled_immigrant": list(row.pooled.immigrant),
                              "pooled_non_permanent_resident":
                                  list(row.pooled.non_permanent_resident)}
