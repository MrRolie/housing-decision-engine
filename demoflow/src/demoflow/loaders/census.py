...
VINTAGE AT THE BOUNDARY: both loaders return their rate sub-tree and DROP `_provenance`, and
that is the whole contract. Spec §7's `data_vintage.source_hashes` is filled by
`pipeline._source_hashes`, which hashes each derived artifact's BYTES off disk and reads that
artifact's own recorded `extracted_at` out of the bytes it hashed — so the envelope names the
vintage the run actually READ, from one declaration site. The run-6 carry (2026-08-13) answered
the same question with a typed `RateVintage` and a vintage accessor per surface here; those and
`loaders/vintage.py` itself were DELETED at the round-3 elegance audit (2026-08-22), because
Task 29 had filled the envelope the other way and nothing outside `tests/` ever called them.
Restoring them would be a SECOND declaration of what the envelope publishes.
"""
import csv
import json
import math
...
# constituents THEN divide — never an average of rates. Every band is an EXACT union of
# published members, so re-aggregating any run of them reproduces the coarser rate they
# replace; that identity is what makes the refinement below ARITHMETIC rather than a remodel,
# and it is the property `test_census_ownership.py` re-anchors the retired 75+ oracle on.
#
# SEVEN BANDS, NOT FOUR (operator ruling W, 2026-08-20; spec §8 amendment #18). Until this
# change the lattice was `25-54 / 55-64 / 65-74 / 75+`, and its 25-54 band was THIRTY YEARS
# WIDE — the same defect class ruling V retired the flat headship curve over: a coarse band
# flattening an age gradient THIS SAME extract publishes finer. Measured on the pinned extract:
# `25-54` served 0.5119964493642796 at MTL_RMR, so `demand/formation.py` multiplied age-27
# household formation by 0.5120 while the extract's own `25 to 29 years` cells publish
# 0.2595837016796593 (32,300/124,430) — a 1.97x OVERSTATEMENT at the ages carrying ~66% of
# horizon `D_native`. The refinement also SPLITS the retired `75+`, which this cube publishes
# as TWO members: QC_RMR runs 0.559783363421747 at 75-84 against 0.4405 at 85+, an 11.9pp
# gradient the flat 0.5338550157591566 hid, in the model's most aging-sensitive region. Those
# counts are sound rather than small-count noise (>= 10,000 households at every 85+ cell; QC's
# 0.4405 is 4,405/10,000, a round-to-5 artifact of an exactly-round denominator).
#
# WHY TEN-YEAR AND NOT FIVE-YEAR, WHICH THIS CUBE ALONE WOULD SUPPORT. These EDGES are shared
# with `hors_aligned.CD_BAND_SPEC` and `hors_aligned.assert_band_lattice` REQUIRES the two to
# be identical — the constituents cannot be shared because the two cubes publish different
# granularities of the same partition, so the edges are the only common object. The CSD cube
# that curve reads, 98-10-0232-01, publishes only EIGHT age members and is TEN-YEAR inside
# 25-54. That is GATED on the cube's own age-member index, not merely recorded, by
#   tests/test_hors_aligned_ownership.py
#     ::test_p11_1_band_lattice_is_the_models_own_and_the_two_cubes_partition_it_alike
# which reds if a re-pin introduces a five-year member inside the modeled span. So a five-year
# refinement here would leave HORS_RMR the ONLY unrefined geography — which is exactly the
# non-common-mode artifact this change exists to REMOVE. Measured with HORS_RMR held coarse:
# five of the eight ranked rows move and HORS_RMR falls 4 -> 6; under this common 7-band
# partition it refines with everything else, holds rank 4, and only ONE rank pair swaps.
#
# `75-84` and `85+` are each a SINGLE published member, so neither is a union any more and the
# retired `75+`'s "both must be present or the derivation raises" no longer has a subject.
# `85+` keeps hi=200 for the reason the retired band did: the lattice's open band closes on a
# sentinel above any age a lookup can carry, never on a published edge.
#
# THE SUB-25 FLOOR IS UNTOUCHED BY THIS (see `OWNERSHIP_LATTICE_FLOOR`). Extending the lattice
# below 25 is amendment #12's other half and is NOT in ruling W: it stays a separate decision,
# and the sub-25 clause of `_zero_support_note` still stands word for word.
_AGE_BAND_SPEC = (
    ("25-34", 25, 34, ("25 to 29 years", "30 to 34 years")),
    ("35-44", 35, 44, ("35 to 39 years", "40 to 44 years")),
    ("45-54", 45, 54, ("45 to 49 years", "50 to 54 years")),
...
def _read_verified_ownership(data_dir: Path | None) -> tuple[dict, Path]:
    """Read + FULLY verify the ownership artifact ONCE — provenance, then the strict join.

    EVERY REFUSAL CAUSE LIVES ON THIS PATH RATHER THAN IN THE ACCESSOR, which is a property
    worth keeping now that the accessor is the only caller. Measured 2026-08-14, when a second
    accessor existed and the strict join sat inside `load_ownership_rates`: a dropped geography
    (or a dropped `rates` key) refused the rates and served the other accessor a confident
    answer, so three of the four causes split the two legs apart. That accessor is gone (see
    VINTAGE AT THE BOUNDARY above); the position of the check is what makes a refusal TOTAL for
    whatever reads this artifact next, and the split it prevents is dated rather than
    hypothetical.
    """
...
# ===========================================================================================
# THE BANDED AGGREGATE OVER A LUMPED AGE RANGE (operator ruling X, 2026-08-21)
# ===========================================================================================
#
# WHY A SECOND ACCESSOR EXISTS AT ALL. `ownership_rate` answers "the rate AT an age" and is
# correct for every consumer that reads ownership PER AGE. A consumer that needs the rate over
# a LUMPED RANGE — one number multiplying a whole span at once — cannot get it from that
# accessor: reading the range's midpoint returns ONE band's rate, and under the seven-band
# lattice (operator ruling W) that is a sub-band of the span, not the span. Ruling W did not
# create that class of consumer; it removed the coarse band that had been silently SERVING it.
# Until the refinement, `25-54` and `75+` existed as bands, so a point read inside either
# returned the very household-weighted union a lumped consumer wanted, and the two questions
# were indistinguishable in the numbers. They are distinguishable now, and the difference is
# material: at MTL_RMR the 25-54 union is 0.511996 while its middle band alone is 0.540707.
#
# THE AGGREGATE IS FORMED FROM COUNTS, NEVER FROM RATES. Owner counts and total counts are
# summed across the constituent members and divided ONCE — the same rule `_AGE_BAND_SPEC`'s own
# header states for a band, applied one level up. A MEAN OF BAND RATES IS NOT A RATE: measured
# on the pinned extract, the unweighted mean of MTL_RMR's 25-34/35-44/45-54 rates is 0.501945
# against the true union 0.511996, off by -1.005 pp — the bands carry materially different
# household counts (277,415 / 343,800 / 336,360), so a rate mean weights them equally when the
# construction weights them by size.
#
# THE RANGE MUST BE AN EXACT UNION OF WHOLE MODEL BANDS, and that is a REFUSAL rather than a
# convention (`bands_spanning`). A range that half-covers a band has no aggregate this cube can
# form, and the failure would be silent: a plausible fraction measured over a population that
# is not the one asked for. It is also the guard that makes this accessor survive the next
# lattice change — a refinement that moved the 25 or 54 edge REFUSES here instead of quietly
# serving a shorter span.

def bands_spanning(lo: int, hi: int) -> tuple[tuple[str, int, int, tuple[str, ...]], ...]:
    """The `_AGE_BAND_SPEC` entries that tile [lo, hi] EXACTLY — refusing any other range.

    Shared with `loaders/hors_aligned.py`, which needs the same band LABELS over the same span
    for the CD cube's own counts. The two cubes cannot share constituents (they publish
    different granularities of one partition) but they DO share edges — `assert_band_lattice`
    holds them identical — so the tiling is computed once here and the label list is what
    crosses the module boundary. A second implementation there could tile the same range
    differently and both would look right.
    """
    inside = tuple(band for band in _AGE_BAND_SPEC if lo <= band[1] and band[2] <= hi)
    if not inside:
        raise LoaderError(
            f"no modeled ownership band lies inside ages {lo}-{hi} — the lattice is "
            f"{[b[0] for b in _AGE_BAND_SPEC]} and an aggregate over a span it does not "
            "tile has no construction")
    if (inside[0][1], inside[-1][2]) != (lo, hi):
        raise LoaderError(
            f"ages {lo}-{hi} are not an EXACT union of modeled ownership bands: the bands "
            f"inside that span run {inside[0][1]}-{inside[-1][2]} "
            f"({[b[0] for b in inside]}). A partly-covered band cannot be aggregated — the "
            "result would be a plausible fraction over a population nobody asked for")
    for prev, nxt in zip(inside, inside[1:]):
        if nxt[1] != prev[2] + 1:
            raise LoaderError(
                f"ages {lo}-{hi} are tiled by bands {prev[0]} and {nxt[0]} with a gap or "
                "overlap between them — the lattice is not a partition over this span")
    return inside


def ownership_union_rates(lo: int, hi: int, data_dir: Path | None = None) -> dict[str, float]:
    """{geography: owner households / total households over ages [lo, hi]}, from the extract.

    THE AGGREGATE A LUMPED CONSUMER NEEDS, formed the way every rate in this module is formed:
    counts summed across the span's published members, divided once. Same three sources as
    `derive_ownership_from_csv` and the same borrowing, so the two surfaces cannot disagree
    about what a geography IS — the CMA rows for MTL_RMR/QC_RMR, the six-CMA net for HORS_RMR,
    and the five RA-level rows borrowing MTL_RMR's value with it.

    IT READS THE PINNED EXTRACT, NOT THE DERIVED ARTIFACT, and that is forced rather than
    chosen: `ownership_by_geo_age.json` publishes band RATES and no counts, and a union cannot
    be recovered from rates without the household weights. `read_totals_cube` verifies the pin
    before returning a cell, so the PIT chain is the same one the derivation runs on — this is
    a second READ of one extract through the one public seam, which is the case that seam's own
    docstring exists for, and never a second derivation of the shipped curve.

    CALL IT ONCE PER RUN. The extract is ~10 MB of CSV; the pipeline builds this map when it
    builds its ownership reader and the ED path reads the resolved map.
    """
    members = tuple(m for band in bands_spanning(lo, hi) for m in band[3])
    cube = read_totals_cube((data_dir or DATA_DIR) / CENSUS_EXTRACT)
    span = f"{lo}-{hi}"
    rates: dict[str, float] = {}
    for geo, source in ((Geography.MTL_RMR, _MTL_CMA), (Geography.QC_RMR, _QC_CMA)):
        ctx = f"ownership_union[{geo.value},{span}]"
        rates[geo.value] = _rate(*_band_counts(cube, source, members, ctx), ctx=ctx)
    ctx = f"ownership_union[{Geography.HORS_RMR.value},{span}]"
    rates[Geography.HORS_RMR.value] = _rate(*net_of_qc_cmas(cube, members, ctx), ctx=ctx)
    for geo, parent in _BORROWS_FROM.items():
        rates[geo.value] = rates[parent.value]
    missing = [g.value for g in Geography if g.value not in rates]
    if missing:
        raise LoaderError(f"ownership union over {span} emitted no rate for: {missing} "
                          "(strict join, same as the banded derivation's)")
    return rates
...
def _zero_support_note(under_15_persons: float, youngest_member: tuple[str, int],
    ...
    return (
        ...
        "rate. That sentence is retired by the ruling-V commit that landed the age-resolved "
        "headship curve (operator ruling V, 2026-08-19); "
        ...
        "the SAME extract while `_AGE_BAND_SPEC` starts the ownership lattice at 25 — its lowest "
        "band is `25-34` since operator ruling W (2026-08-20) refined the lattice to seven bands, "
        "and was `25-54` before it; the FLOOR is 25 on both and ruling W did not move it, which is "
        "why this clause is untouched by that refinement — so the two "
...
def _read_verified_headship(data_dir: Path | None) -> tuple[dict, Path, dict[str, dict[int, float]]]:
    """Read, verify AND strictly join the headship artifact ONCE, for every accessor below.
    ...
    check that ran only where the rates are read let the SECOND accessor of this artifact serve
    a confident answer for a curve no run may use. That second accessor was the vintage reader
    and it is deleted (see VINTAGE AT THE BOUNDARY above); the position of the check stays,
    because it is what makes a refusal total for every caller rather than for one of them.
    """
...
def load_headship_rates(data_dir: Path | None = None, *, shape: str) -> dict[int, float]:
    """ONE shape's curve as {int age: rate}. `shape` is REQUIRED and keyword-only.

    IT USED TO DEFAULT TO THE ARTIFACT'S OWN `central_shape`, and that default was a SECOND
    shape-selection site — reachable, unhashed, and pointed at a value `assumptions_hash` does
    not cover. The run's selection lives in `CENTRAL_ASSUMPTIONS["headship_shape"]` (headline)
    or a sweep leg's endpoint; a caller who states nothing must get a TypeError rather than a
    curve chosen by the file. Closed at the round-3 elegance audit (2026-08-22): the default had
    no non-test caller to protect, and the hazard it carried was live — the artifact's
    `central_shape` and the constant are bound together by test today, so a silent divergence
    would move the numbers under a byte-identical identity token.
    """
    _payload, _path, curves = _read_verified_headship(data_dir)
    return headship_curve(curves, shape)
