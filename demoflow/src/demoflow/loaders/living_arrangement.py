...
coupled_s ≤ its sex pool, and the 75+ aggregate direction) fire on MODELLED counts in the
initialization task and raise `CalibrationError`. The one direction check that lives HERE is
a different animal with a different error class — see `_assert_coupled_direction`.
"""
import json
