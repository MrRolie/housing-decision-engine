...
uv run hde --refresh-plan                                   # what expires, what to re-read, where (exit 3 if lapsed)
...
- **Anchors survive their author** (2026-09-22, board item 6) — a `valid_until` is a promise the figure will be replaced, so `__post_init__` refuses a dated anchor that carries no `refresh_group` (which publishing RELEASE prints it), no `quoted` (how that source printed the figure, so the next edition can be diffed against it) or no `unit`. `anchors.REFRESH_SOURCES` holds one record per release — publisher, the edition to look for, where it appears, when it was last checked and what was found — and derives its members from `Anchor.refresh_group`, so membership has one home. `anchors._RELEASE_EDITION` holds one `(as_of, retrieved_on, valid_until)` row per release and every grouped anchor spreads `**_edition(group, dated=…)`: a release is the unit a refresh moves, and before this five releases shared `_TAX_RETRIEVED`, so refreshing the CRA's figures in November silently restated Québec's as read that day. Never give a grouped anchor a date of its own. `hde --refresh-plan` prints the work order and exits 3 once a figure has lapsed; `anchors.REFRESH_PROCEDURE` is the one home of how a refresh is done. A new edition REPLACES the figure in place (`replaces`, with `why` saying scheduled-indexation rather than correction); `restatements` is the same figure in another convention and is never where a new year's figure goes. `tests/test_anchor_refresh.py` turns red on a value moved without its quote, a vintage moved without its validity date, or one sibling moved without the rest. Spec: `docs/specs/2026-09-22-anchor-refresh-path.md`.
