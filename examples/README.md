...
in cash); `first_time_buyer: true` is recorded and the run says it applied nothing at
closing — Montréal's own acquisition program closed to new applications on 2026-07-07, and
Québec's first-home refund (up to $5,875 of the duty) is a tax credit paid on the return,
which the line names by anchor rather than booking as cash the engine cannot verify. The
