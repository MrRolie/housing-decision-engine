...
Five walkthrough scenarios follow, in reading order, plus `advanced_config.yaml` — the
...
Acts render by what the config supports: acts 1–4 always; act 5 ("Why") only with a
`market_scenario:` prior; act 6 ("The market line", the break-even rent) only with `rent`
plus an owned option.
...
Acts 1–4 and 6, now three runners in the race. Watch act 2: renting starts cheapest (no down payment) and the crossover year — if any — is the whole decision.
...
are non-zero only here. `mortgage_rate` is an EFFECTIVE ANNUAL rate with annual payments; a
posted Canadian rate compounds semi-annually — convert it first (the schema note and the
figure glossary state the formula).
...
| Discount rate (0.03–0.05 real) | illustrative — personal time-preference; sanity band [0.02, 0.06]; the engine warns outside [0, 0.15] (units tripwire) | `discount_rate` in all examples |
