...
**Architecture:** `demoflow/` is its OWN uv project inside the repo (own `pyproject.toml`, lockfile, venv, tests), sibling to `src/hde/` + `mcp_server/`. It takes a uv **path dependency on an external actuarial package** (declared in its own `pyproject.toml`) for the CPM2014/CPM-B mortality engine (`mcp_server.engine.mortality`) and installs that package's `mcp_server` distribution — it NEVER installs hde's distribution. The two envs are never shared: hde's repo also ships a top-level `mcp_server`, so co-installation would collide; isolation is by construction, not discipline. Coupling to hde is the future ScenarioPrior artifact file only (Tranche 2).
...
[tool.uv.sources]
actuarial-system = { path = "../..<the external actuarial checkout>" }
...
>
> **Dispatch a data-integrity review agent on this task** — the re-triage debt in
> the internal verification record for this tranche. This is the first CONSUMER read of
...
- Write: the verdict to the internal verification record (quant review).
...
- [ ] **Step 4: Commit the verdict** to the internal verification record.
...
- Write: the report to the internal verification record (stress review).

- [ ] **Step 1: Dispatch agent.** Use the `Agent` tool with an adversarial stress-testing agent. Hand it the plan
...
- [ ] **Step 4: Commit the report** to the internal verification record.
...
- Write: the verdict to the internal verification record (data-integrity review).
...
- [ ] **Step 3: Commit the verdict** to the internal verification record.
