...
| `warnings` | coherence warnings + time-anchor violations + affordability breaches + the decisiveness-provenance warning (`decisiveness rests on <lead>: …` — fires only under the `mc_floor` verdict rule, names every uncertainty input that is `assistant` or unattributed with its value, and leads with the class that fired: `uncertainty inputs the user did not state` for the assistant's own figures, `uncertainty inputs no sources: entry attributes — typed in the config with nobody's name on them, so the engine cannot say whose figures they are` when every named input is undeclared, and `uncertainty inputs the assistant typed or that no sources: entry attributes` for a mix — an undeclared figure is one nobody claimed, never one the user is known not to have typed, and closes with what the deterministic line alone says and whether that margin clears the tie band), the same list the CLI prints to stderr; also (2026-09-04) the prior-without-Monte-Carlo warning — `market_scenario prior acts only in Monte Carlo — this run shows the deterministic line alone (the prior's drift is not in it)`, on `--no-monte-carlo` with a `market_scenario` block, with or without sweeps — and the one-sided sweep warning — `sweep of <key> covers only values ABOVE\|BELOW the placeholder <base>; the other direction is untested`, when a `--sweep` over a key declared `assistant` in `sources:` lies entirely on one side of the config's value — and (2026-09-08) the validity warning — `anchor <name> (<value>) is past its validity date <valid_until>: the source says the figure changes after that date — re-read <short_cite> before relying on the run`, once per anchor the run USED (an applied default, an `anchor:` declaration in `sources:`, a schedule the loader priced — the insured mortgage's band and premium tax, the transfer-tax brackets the price reached, the brackets a `tax:` block's income reached and the limits its legs read) whose `valid_until` falls before the run date; silent through the date itself, and a reference entry the run never touched never warns; and (2026-09-08) the nominal-zero growth warning — `<option>.value_growth_rate 0.0% as quoted is a 2.1%/yr REAL decline in the home's value; a "flat" view that tracks inflation is 2.1% quoted — or omit the key for the neutral 0% real default; say which reading the user meant` — once per owned option whose `value_growth_rate` is TYPED as exactly 0 in nominal mode under the as-quoted convention with a non-zero `inflation_rate` (never on the omitted default, which is the neutral 0% real; never in real mode or under `rates: real`) | — and (2026-09-21) the four renewal warnings, all from `config.coherence_warnings`: the NOT-MODELLED one, `<option>: renewal risk is not modelled — mortgage_rate <r> is held for the whole <A>-year amortization, …`, which fires on EVERY financed option that declares no `mortgage_renewal_years` (so every existing config with a mortgage gains one warning line), and reads differently at `mortgage_term_years <= 5`, where a 5-year term covers the amortization and the mortgage never renews; the INERT one, `<option>.mortgage_renewal_years=<T> is at or past the <A>-year amortization — the mortgage never renews inside its own life, …`; the OUT-OF-HORIZON one, `<option>: the stated renewal ladder prices nothing in this run — the first renewal falls at year <k>, past the <N>-year horizon, …`, when no renewal falls inside `simulation.years` (the run prices `mortgage_rate` throughout and renewal risk is still unmodelled); and the BIAS one, `<option>: the stated renewal path prices the mortgage $<d> below its contract rate mortgage_rate <r> held throughout …`, measured on the PV of the ladder the run PRICED against the same mortgage at its contract rate over the same horizon — not on the declared rate list, which carry-forward and the horizon both make a different path. Plus a fifth, the READING one: `<option>.mortgage_renewal_rates opens at <r>, the same figure as mortgage_rate: the engine reads this list as ONE RATE PER RENEWAL …`, fired when the first declared rate repeats `mortgage_rate` — the signature of the list being read as one rate per five-year BLOCK rather than one per renewal, which shifts the whole path a term later. It names both readings and refuses neither, because holding the contract rate through the first renewal is a real thing to model; it is the one renewal warning that reads the DECLARED list rather than the priced schedule, the gap between them being its subject. The one-sided sweep warning also fires for a LIST placeholder, comparing the grid against the stated path's whole span, and says so explicitly when the placeholder is not a number it can compare at all
| `assumptions` | `mode`, `years`, `discount_rate` (the rate in use: in nominal mode a real figure — the anchored default, or one declared `rates: real` — composed with `inflation_rate`, `(1 + real)(1 + π) − 1`, like every other rate, and a typed as-quoted figure used as typed; `discount_rate_note` says so in one line, `composed at parse: (1 + 3.5% real)(1 + 2.5% inflation_rate) − 1 = 6.09% nominal`, `null` when nothing was converted), the text `lines` of the Assumptions block (the `mode:` line names both figures in nominal mode — `discount_rate 3.5% real → 6.1% nominal (incl. 2.5% inflation)`, `default` inserted when the anchor was used; the `<option> financing:` line carries the down payment, its share of price, the distance to the 20% mortgage-insurance line and the loan-to-value; with `cash_available` it leads with the netting `cash − purchase_costs = down payment` and closes with the price at which that pile stops covering 20% down — `(cash − purchase_costs) ÷ 20%`, with the `purchase_costs` figure it holds fixed; with `mortgage_insurance` active it adds an `insured:` clause — the tier, the financed premium, the provincial tax paid in cash and the resulting loan and loan-to-value — and the loan-to-value it quotes is the tier basis, before the premium; an owned option that priced a transfer tax gets its own `<option> purchase costs:` line — the duty, the schedule or schedules that charged it, the first-time-buyer rebate applied (or the maximum NOT applied, or the fact that none is anchored), and the decomposition of the `purchase_costs` figure the financing line quotes, so the two lines reconcile; it is a separate line because an all-cash purchase has no financing line and pays the tax all the same), `defaults_applied` (one entry per key the YAML omitted: `key`, `value`, `formatted`, `cite`, `kind`, `note` — how `value` relates to `anchor.value` when nominal mode composed it, else `null` — and the full `anchor` record), `reference_matches` (one entry per owned-option `other_recurring_costs` line naming a property tax or home-insurance premium: `option`, `cost_name`, `annual_amount`, `family`, `implied_rate` — the amount as a fraction of `initial_value`, `null` for insurance — `matches`, the full anchor record of every anchor cited for the line, and `citations`, how they combine: one `{kind, anchors, total}` record per claim, `kind` either `single` (one published figure equals the line) or `sum` (a municipal rate plus its province's school rate — the bill a Québec owner actually pays), `anchors` the registry names in citation order and `total` the published figure the line was matched against. Both lists empty means no source agrees, which is reported rather than hidden), and `demographic_prior` (provenance block + `description` + cited `sources`) or `null`, and `sources` — the source-class echo: `declared` (false when the config carries no `sources:` block, and then the lines say so in one sentence), `user` / `assistant` / `unattributed` / `sweep` (each a list of `{key, value, formatted}`, `formatted` in the config's own units; `sweep` is non-empty only in the document of a grid point whose key a `--sweep` or `--break-even` moved, and its line reads `swept: <key>=<value>`) and `anchor` (a `{key: anchor name}` mapping) — and `read_back`, the block below (also `anchor-sourced:` lines, so a cited figure travels with its citation). On the CLI's surfaces — the text report, `--json` and the read-back — the financing line's 20%-down price is solved through the loader (2026-09-04), so a `purchase_costs_rate`, a `land_transfer_tax: auto` schedule and the premium tax are re-derived along the price and the clause reads `(purchase_costs $Y at that price; above it the mortgage is insured)`; a surface holding the spec alone keeps the seed figure and says `purchase_costs held at $Y`; the under-20%-down `[warning]` ends with the same clause (2026-09-08: `… carries interest for the whole amortization; this cash covers 20% down up to a price of $X (purchase_costs $Y at that price; above it the mortgage is insured)`), built once (`serialization.cover_clause`), so the warning and the financing line agree to the dollar on every surface; `rates` (2026-09-05) — `as_quoted` (the default: every typed growth, escalation, return and discount rate is the figure as quoted, converted once at load) or `real` (the config declared its figures real); `inflation_rate` — the deflator of the typed rates in real mode and the rate composed onto the real defaults in nominal mode (the FP Canada 2.1% planning figure when omitted in real mode under `as_quoted`, echoed under `defaults_applied` with the `economic.inflation_rate.nominal_planning` anchor); and `converted_rates` — one `{key, quoted, effective}` per typed rate the loader converted, in read-back order: `quoted` the figure as typed and `effective` the rate the run uses in the run's terms (the deflated real figure in real mode; the quoted figure itself in nominal mode, where the engine composes the stored real figure back to it), an `other_recurring_costs` line keyed `<option>.other_recurring_costs.<name>.escalation_rate`; empty under `rates: real`. `discount_rate_note` has two more forms for a typed rate — `deflated at parse: (1 + 5.0% as quoted)/(1 + 2.1% inflation_rate) − 1 = 2.84% real` (real mode) and `as quoted: 5.0% nominal, used as typed` (nominal mode) — and `lines` gain the `rates:` line: `rates: as quoted · rent.rent_escalation_rate 3.0% as quoted = 0.9% after 2.1% inflation · …` in real mode, `… = 3.0% nominal = 0.9% real` in nominal mode (2026-09-08: the quoted figure in use and its real equivalent, `(1 + quoted)/(1 + π) − 1` — the real-mode clause's own arithmetic, so a nominal `0.0%` for a home's value reads `= 0.0% nominal = -2.1% real`), `rates: real (declared) · typed rates are real figures, …` under `rates: real`, `rates: as quoted · no typed rate to convert` when nothing was typed — and (2026-09-08) closed by one clause per typed `mortgage_rate`, under `rates: real` too, since a mortgage rate is converted by its own compounding and never by inflation: `condo.mortgage_rate 4.95% as quoted (semi-annual) = 5.011% effective annual` under the default `mortgage_rate_compounding: semi_annual` (the Canadian fixed-rate convention the registry's `mortgage_rate.*` restatements encode; the loader converts once, `(1 + r/2)^2 − 1`, and the level payment uses the effective figure), `… 4.95% effective annual, as typed` under `effective_annual`; `mortgage_rates` carries the same — one `{option, quoted, compounding, effective}` per typed mortgage rate, empty otherwise; `mortgage_renewals` (2026-09-21) carries the renewal ladder an owned option declared — one `{option, renewal_years, rates_quoted, rates, compounding, segments}` per laddered option, `segments` a `{start_year, end_year, rate, payment, opening_balance, priced}` row per rate contract, from the engine's own schedule, `priced` False once `start_year` passes `simulation.years` — `_financing_pv` values nothing there — with `renewals_priced` counting the renewals inside the horizon; `rates`/`rates_quoted` are the CONFIG's own echo, the renewal rates as typed WITHOUT the in-force `mortgage_rate` that `segments[0]` runs at, so the two lists are offset by one and a consumer reads the priced path off `segments`; empty when no option declared one, and there is no anchor behind any of it, the rates being a stated scenario; a `sources:` declaration on `mortgage_rate` validates against the anchor's published figure and its restatements alike, so it holds under either convention; the `mode:` line reads `discount_rate 5.0% as quoted → 2.8% real (after 2.1% inflation)` in real mode and `discount_rate 5.0% as quoted, used as typed` in nominal mode for a typed rate, and keeps its composed form for the anchored default and for a rate typed under `rates: real`. Beside `read_back` rides `read_back_short` (2026-09-05, the gist shape): the `[warning]` lines, the source lines and the `decisiveness:` line of `read_back` — a strict subsequence of it, in its order — closed by one engine line counting the lines the full block adds and naming their sections, `full read-back: <n> more lines (defaults applied, financing, year-1 cash, …) — rerun with --read-back full` (absent when nothing was left out; the `rates:` line is among the lines it counts, under the section name `rates`); `--read-back short` prints it alone. Since 2026-09-08 the short block also carries, between the source lines and `decisiveness:`, two lines of its own: `you said:` — the `user` entries of the source echo as `key=formatted`, in the config's own units (`you said: rent.monthly_rent=$1,600/mo, income.annual_income=$70,000, …`), only when `sources:` declares a `user` key; the full block never carries the user's own figures, by design — and, with a `tax:` block, a one-line `tax:` summary (`tax: marginal rate 36.12% (resolved from income in QC) · renter capital sheltered $61,000 / taxable $14,000 (+ FHSA refunds $2,889) · drag $2,340 at year 10 (PV $1,415) charged to rent · owner exempt (principal residence) · FHSA rollover haircut PV $5,744 charged to rent`; each clause only when the run has it — `(as typed)` for a typed rate, `drag none — all sheltered` when nothing sits in the taxable share, the refunds parenthetical only when FHSA refunds join it, no haircut clause without an FHSA share) while the full block keeps its full `tax:` line unchanged. So the short block's shared lines are a subsequence of the full block, its two own lines are not in it, and the closing count is the number of lines the full block has beyond the short body (the full `tax:` section is named among them) |
...
line, so the numbers the ENGINE chose are named with their citations; the
`rates:` line — the convention the typed rates were read under (`as quoted`,
the default, or a declared `real`) and every converted rate in both forms, as
quoted and in use (2026-09-05); in nominal mode the `mode:` line — the
...
used as typed; the `decisiveness:` line; each `<option> financing:` line and each `<option>
...
uv run hde <config.yaml> --read-back short   # the gist shape: [warning] lines, source lines, you said:,
                                             #   the tax: summary, decisiveness:, and one closing line counting the rest
```
...
personal amounts, Ontario surtax thresholds and TFSA limits are annually
indexed 2026 figures; the Home Buyers' Plan grace window ends 2028-12-31),
...
- `warnings`: without the block, when the renter holds capital — `rent: invested
  ...
  attributes the term to the cause that carries it — a rate spread alone, the
  tax alone (in which case it says so and that setting the two rates equal
  changes nothing), or both with their dollar split, each cause naming the
  input that moves IT (`tax.renter_capital` for the taxable share's drag,
  `tax.retirement_marginal_rate` for the FHSA rollover haircut) — and
