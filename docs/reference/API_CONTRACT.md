...
| `sweeps` | only with `--sweep`: one entry per flag — `key`, `values`, per-point `rows` (`value`, `totals` per option, the verdict fields `best` / `runner_up` / `margin_pv` / `margin_frac` / `decisive` / `prob_best` / `mc_mean_best` / `reason`, or `error` when that point is refused) and `flips` (consecutive points whose cheapest option differs) plus `mc_mean_flips` (the same for `mc_mean_best`) |
| `break_evens` | only with `--break-even`: one entry per flag — `key`, the two `options`, the `bracket` searched, `base_value`, `tie_band_fraction`, and `break_evens` (each: `value` where the deterministic totals cross, `cheaper_below` / `cheaper_above`, `tie_band` edges `[lo, hi]` — `null` when an edge lies outside the bracket); `cheaper_throughout` when there is no crossing |
