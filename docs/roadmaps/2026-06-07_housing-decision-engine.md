...
- **2026-09-02 (evaluation rounds 1–4):** the skill routes every financed purchase to `mode: nominal` (the lender collects the nominal payment; a real-rate level payment hid GDS breaches of 33% and 36% against 32% in two trial runs) — reversible, verdict drift measured at $83.2k → $81.9k and 3.1% → 2.9%, and it interacts with the open nominal-semantics ruling below; the S4b nominal-mode refusal of a prior was lifted (the drift is real and composes)..
...
### Backlog from the 2026-09-03 fresh-shape evaluation (five question shapes; engine gaps the reviews hit)
...
### Backlog from the 2026-09-02 user-model evaluation (engine gaps the trial runs hit; operator rulings needed where marked)

- **Tax treatment of the two sides' money — SETTLED 2026-09-05 (asked as "should we model the FHSA?"):** tax asymmetry first, the FHSA as its first-home slice. The intake asks where the renter's savings sit (TFSA, RRSP, FHSA, taxable); the engine applies a tax drag to the unsheltered share from anchored federal and provincial (QC, ON) brackets and names the owner's principal-residence exemption; for a `first_time_buyer` the FHSA's refunds and a Home Buyers' Plan withdrawal enter day-one cash with the plan's repayment cost. Every bracket edge, rate and limit is a live-fetched anchor. Why: today neither side is taxed and only the skill's not-modelled line names the bias; the renter-side drag is the size of served verdict margins, the FHSA refund a small slice of it. Two folds; a design note in `docs/specs/` precedes the code.
- **Rate convention — SETTLED 2026-09-05 (supersedes the typed-rate half of the 2026-09-04 ruling):** users type STICKER rates, as quoted — rent escalation, price growth, investment return, discount rate, mortgage rate — and the engine converts once at load (deflated in real mode, as typed in nominal mode), printing both forms in the read-back ("3% as quoted = 0.9% after 2.1% inflation"). Anchors stay real and print composed. Every example migrates to as-quoted values; PROMPTS.md carries one sentence on the difference. Why: the served rounds showed assistants converting sticker numbers by hand and inflating twice.
- **The gist shape's block — SETTLED 2026-09-05:** a user who asked for the gist gets a SHORT read-back (the warnings, the source lines, the decisiveness line) and one line offering the full block; every other shape keeps the full block. Why: 203 words of answer followed by ~1,000 of block for a user who said "don't write me an essay".
- **No mortgage quote — SETTLED 2026-09-05:** the run uses the contracted 5-year rate (Bank of Canada funds-advanced, uninsured / insured) as the base and sweeps to the posted rate as the ceiling; the verdict sentence names the rate. Ratifies the round-10 skill fold.
- **Nominal-mode semantics — SETTLED 2026-09-04:** a typed `discount_rate` in nominal mode is a REAL opportunity cost the engine composes with inflation (a typed 3.5% runs at 5.7% nominal), the rule `investment_return_rate` and every escalation rate already follow; only quoted contract rates (a mortgage) stay as typed. The "typed rate below the composed rate" tripwire is retired, `examples/advanced_config.yaml` is relabelled real, and the skill's gate 3 may state a real rate. Previously: discount and mortgage rates were used as entered, so a copied real anchor discounted nominal flows at a real rate and only a warning said so.
...
- **Decisiveness vs the Monte Carlo majority — SETTLED 2026-09-04:** the verdict has three states — an option (decisive or not), too close to call, or a NAMED disagreement ("the best guess says rent, most futures say house") that is never decisive; both figures reach the console table, the sweep rows, the read-back `decisiveness:` line and the story's first act. Nothing changes when the two agree. Previously `best` was the deterministic winner and a 66% majority for the other option printed as "rent, not decisive" beside a column that said house.
